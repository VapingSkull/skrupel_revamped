...
if ($spielanzahl>=1) {
    ...
    for ($i=0; $i<$spielanzahl; $i++) {
        ...
        for ($j=1; $j<=10; $j++) {
            if ($spieler_id_c[$j] > 0) {
                $zeiger_temp= @mysql_query("SELECT * FROM $skrupel_user WHERE id='" . $spieler_id_c[$j] . "' order by id");
         ...
         <tr><td><form name="formular"  method="post" action="<?php echo servername;?>admin/spiel_gamma.php?fu=2&slot_id=<?php echo $slot_id?>" onsubmit="return confirm('Das Spiel \'<?php echo $name; ?>\' wirklich l&ouml;schen?');"></td>
