...
$zeiger = @mysql_query("SELECT * FROM " . table_prefix . "user where uid='" . $uid . "'");
...
if ($datensaetze==1) {
    ...
    $zeiger2 = @mysql_query("SELECT * FROM " . table_prefix . "spiele where sid='$sid' and (spieler_1=$spieler_id or spieler_2=$spieler_id or spieler_3=$spieler_id or spieler_4=$spieler_id or spieler_5=$spieler_id or spieler_6=$spieler_id or spieler_7=$spieler_id or spieler_8=$spieler_id or spieler_9=$spieler_id or spieler_10=$spieler_id) order by sid");
    ...
    if ($datensaetze2==1) {
        ...
        if ($phase==1) {
            if ((preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_NAME'])) or (preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_FILENAME']))) {
                header("Location: " . servername . "inhalt/kommunikation_exch.php?fu=7");exit;
            } else {
                header("Location: " . servername ."inhalt/runde_ende.php?fu=1&spiel=" . $spiel . "&sprache=".$spieler_sprache);exit;
            }
        }
        ...
        if ((preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_NAME'])) or (preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_FILENAME']))) {
            ...
            header("Location: " . servername ."index.php?sprache=".$spieler_sprache); exit;
        }
    }
} else {
    if ((preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_NAME'])) or (preg_match ("/kommunikation_exch/i",$_SERVER['SCRIPT_FILENAME']))) {
        ...
        header("Location: " . servername ."index.php"); exit;
    }
}
