 ...
 */
$lang1 = get_phrasen('de', 'flottealpha');
$lang2 = get_phrasen('de', 'spionagen');
...
if ($fuid==1) {
    ...
    $zeiger = "SELECT fracht_min2,kox,koy,flug,warp,zielx,ziely,zielid,antrieb,mission,masse_gesamt,lemin FROM " . table_prefix . "schiffe use index (PRIMARY) where id='".$shid."'";
    ...
    if ($antrieb==0) {
        ?>
        <body text="#000000" scroll="auto" style="background-image:url('<?php echo servername;?>bilder/aufbau/14.gif'); background-attachment:fixed;" bgcolor="#444444" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
            <br><br><br>
            <center><?php echo $lang['flottealpha']['keinantrieb']; ?></center>
            <?php
    } else {
        ?>
        <body text="#000000" scroll="auto" style="background-image:url('<?php echo servername;?>bilder/aufbau/14.gif'); background-attachment:fixed;" bgcolor="#444444" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="rechnen();">
        <?php
        ...
        if ($flug==0) {
            ...
            /*$zeiger_temp =@mysql_query( "SELECT id,name FROM " . table_prefix . "planeten where id='".$zielid."'");
            $array_temp = @mysql_fetch_array($zeiger_temp);
            $planeten_name=$array_temp["name"];*/
            $zeiger_temp = "SELECT name FROM " . table_prefix . "planeten where id='".$zielid."'";            
            $planeten_name=$Db->GetOne($zeiger_temp);
            ...
            /*$zeiger_temp = @mysql_query("SELECT id,name FROM " . table_prefix . "schiffe where id='".$zielid."'");
            $array_temp = @mysql_fetch_array($zeiger_temp);
            $schiff_name=$array_temp["name"];*/
            $zeiger_temp = "SELECT name FROM " . table_prefix . "schiffe where id='".$zielid."'";
            $schiff_name = $Db->GetOne($zeiger_temp);            
        ...
        <script language="JavaScript">
            ...
            <table border="0" cellspacing="0" cellpadding="1" width="100%">
                <tr>
                    <td colspan="12"><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="30" height="5"></td>
                </tr>
                <tr>
                    <td colspan="8" rowspan="2" width="100%">
                        <table border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td><form name="formular" id="formular" method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=2&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>"></td>
                    ...
                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="10" height="1"></td>
                    <td></td>
                ...
                <tr>
                    <td><a href="javascript:hilfe(16);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                    <td></td>
...
if ($fuid==2) {
    include (inhalt_dir . "inc.header.php");
    ?>
    <body text="#000000" scroll="no" style="background-image:url('<?php echo servername;?>bilder/aufbau/14.gif'); background-attachment:fixed;" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        <?php
        $warpfaktor=intval($params["warpfaktor"]);
        $flug=intval($params["flug"]);
        $lemin=intval($params["lemin"]);
        $zielxf=intval($params["zielxf"]);
        $zielyf=intval($params["zielyf"]);
        $koxf=intval($params["koxf"]);
        $koyf=intval($params["koyf"]);
        $zielidf=intval($params["zielidf"]);
        $verbrauchf=intval($params["verbrauchf"]);
        $benzin2=intval($params["benzin2"]);
        $kursmodus=intval($params["pathfind"]);
        $masse_gesamt=intval($params["masse_gesamt"]);
        ...
        $Db->Execute("UPDATE " . table_prefix . "schiffe set flug='".$flug."',warp='".$warp."',plasmawarp='0',zielx='".$zielx."',ziely='".$ziely."',zielid='".$zielid."' where id='".$shid."' and besitzer='".$spieler."'");
        $Db->Execute("UPDATE " . table_prefix . "schiffe set routing_schritt='0',routing_status='0',routing_koord='',routing_id='',routing_mins='',routing_warp='0',routing_tank='0',routing_rohstoff='0' where id='".$shid."' and besitzer='".$spieler."'");
        ?>
        <br><br>
        <?php echo $message; ?>
        <script language="JavaScript">
            ...
            if($kursmodus) {
                if ($flug==0) {
                    ...
                    $planeten_name = $Db->getOne("SELECT name FROM " . table_prefix . "planeten where id='".$zielid."'");
                    //$array_temp = @mysql_fetch_array($zeiger_temp);
                    //$planeten_name=$array_temp["name"];
                    ?>
...
if ($fuid==3) {
    include (inhalt_dir. "inc.header.php");
    // include (lang_dir . $spieler_sprache."/lang.spionagen.php");
    
    $Db->setFetchMode(ADODB_FETCH_BOTH);
    $zeiger = "SELECT * FROM " . table_prefix . "schiffe use index (PRIMARY, besitzer) where id='".$shid."' and besitzer='".$spieler."'";
    $array = $Db->getRow($zeiger);    
    $status=$array["status"];
    $kox=$array["kox"];
    $koy=$array["koy"];
    ...
    $Db->setFetchMode(ADODB_FETCH_BOTH);
    $sql = "SELECT * FROM " . table_prefix . "planeten where besitzer='".$spieler."' and spiel='".$spiel."' and x_pos='".$kox."' and y_pos='".$koy."'";    
    $zeiger2 = $Db->execute($sql);
    $planeten_anzahl = $zeiger2->recordCount();
    $akademie=0;
    if($planeten_anzahl==1){
        $array2 = $Db->getArray($zeiger2);
    ...
    ?>
    <body text="#000000" style="background-image:url('<?php echo servername;?>bilder/aufbau/14.gif'); background-attachment:fixed;" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        ...
        <center>
            <table border="0" cellspacing="0" cellpadding="1">
                ...
                <tr>
                    ...
                    <td><a href="javascript:hilfe(13);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                </tr>
            </table>
        </center>
        <center>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td><form name="formular" id="formular"  method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=4&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>"></td>
                    <td><input type="radio" name="aktion" value="0" style="width:20px;" <?php if ($spezialmission==0) { echo "checked"; }?>></td>
                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="3"></td>
                    ...
                    <td>
                        <table border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                ...
                                <td align='right';><a href="javascript:hilfe(18);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                            </tr>
                        </table>
                    </td>
                    <td></td>
                    ...
                    <td>
                        <table border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                ...
                                <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                <td><a href="javascript:hilfe(20);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                            </tr>
                        </table>
                    </td>
                    <td></td>
                    ...
                    <td></td>
                        ...
                        <td><table border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                ...
                                <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                <td><a href="javascript:hilfe(34);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                            </tr>
                        </table>
                    </td>
                    <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(19);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(21);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(39);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(22);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(23);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><?php echo $lang['flottealpha']['auto']; ?></td>
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(24);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                    ...
                    <tr>
                        ...
                        <td><table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(25);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                        </tr></table></td>
                        <td></td>
                    </tr>
                    <?php
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(26);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(27);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="4" height="1"></td>
                                    <td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(28);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                    ...
                    <tr>
                        ...
                        <td><table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(29);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                        </tr></table></td>
                        <td></td>
                    </tr>
                    <?php
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="6" height="1"></td>
                                    <td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="7" height="1"></td>
                                    <td>
                                        <select name="crewmen">
                                        <?php
                ...
                if (($antrieb>=3) and ($antrieb<=7)) {
                    ...
                    $schiffanzahl=0;
                    $zeiger_2 = @mysql_query("SELECT id,kox,koy,spiel,masse,besitzer,name FROM " . table_prefix . "schiffe where id<>".$shid." and kox='".$kox."' and koy='".$koy."' and masse<=".$maxmasse." and besitzer='".$spieler."' and spiel='".$spiel."'");
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe(40);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                        ...
                        <td>
                            <script language=JavaScript>
                                function hilfe_spionage_missionen() {
                                    ...
                                    window.open('<?php echo servername;?>inhalt/hilfe_spionage.php?fu2=3&spid='+spid+'&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>','Hilfe','resizable=yes,scrollbars=no,width=480,height=200,top='+oben+',left='+links);
                                }
                            </script>
                        </td>
                        ...
                        <td>
                            <table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                    <td><a href="javascript:hilfe_spionage_missionen();"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                                </tr>
                            </table>
                        </td>
                        <td></td>
                ...
                $zeiger = @mysql_query("SELECT * FROM " . table_prefix . "politik where (partei_a='".$spieler."' or partei_b='".$spieler."') and status='5' and spiel='".$spiel."'");
                                        ...
                                        <select name="spielerid" style="width:140px;">
                                            ...
                                            for  ($i=0; $i<$allianzahl;$i++) {
                                                ...
                                                if ($partei_a==$spieler) {
                                                    ...
                                                    $nick = $Db->getOne("SELECT nick FROM " . table_prefix . "user where id='".$partei_b_id."'");                                                    
                                                    ?>
                                                    ...
                                                    $nick = $Db->getOne("SELECT nick FROM " . table_prefix . "user where id='".$partei_a_id."'");                                                    
                                                    ?>
                ...
                if ($antrieb>=1) {
                    $zeiger2 = @mysql_query("SELECT id,spiel,name FROM " . table_prefix . "schiffe where id<>'".$shid."' and spiel='".$spiel."' and besitzer='".$spieler."' order by name");
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select name="warpfaktor">
                                                ...
                                                for($i=1;$i<10;$i++){
                                                    ?>
                                                    <option value="<?php echo $i."\""; if($antrieb<$i){ echo "style='background-color:#aa0000'; "; } if(($warp==$i) and ($flug==4)){ echo "selected"; } echo ">".str_replace('{1}',"$i",$lang['flottealpha']['warp']); ?></option> <?php echo "\n";?><?php } ?>
                                            </select>
                                        </td>
                                        <td><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="3" height="1"></td>
                                        <td><a href="javascript:hilfe();"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                ...
                <tr>
                    <td colspan="5"><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="1" height="4"></td>
                </tr>
            </table>
        </center>
        </div>
...
if ($fuid==4) {
    ...
    <body text="#000000" scroll="no" style="background-image:url('<?php echo servername;?>bilder/aufbau/14.gif'); background-attachment:fixed;" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        <?php
        $begleitschutz=@intval($params["begleitschutz"]);
        $crewmen=@intval($params["crewmen"]);
        $projektile=@intval($params["projektile"]);
        $zeiger = @mysql_query("SELECT id,kox,koy,flug,zielid,warp FROM " . table_prefix . "schiffe where id='".$shid."'");
        $array = @mysql_fetch_array($zeiger);
        $kox=$array["kox"];
        $koy=$array["koy"];
        $flugalt=$array["flug"];
        ...
        $warpalt=$array["warp"];
        $spezialmission=@intval($params["aktion"]);
        $betamodus=@intval($params["betamodus"]);
        $viralziel=@intval($params["viralziel"]);
        if ($spezialmission==0) {
            $message=$lang['flottealpha']['speznachricht0'];
        } elseif ($spezialmission==1) {
            $message=$lang['flottealpha']['speznachricht1'];
        } elseif ($spezialmission==2) {
            $message=$lang['flottealpha']['speznachricht2'];
        } elseif ($spezialmission==3) { 
            $message=$lang['flottealpha']['speznachricht3'];
        } elseif (($spezialmission==4) and ($betamodus<>1)) { 
            $message=$lang['flottealpha']['speznachricht4'];
        } elseif (($spezialmission==4) and ($betamodus==1)) {
            $spezialmission=27;
            $message=$lang['flottealpha']['speznachricht27'];
        } elseif ($spezialmission==5) {
            $message=$lang['flottealpha']['speznachricht5'];
        } elseif (($spezialmission==6) and ($betamodus<>1)) {
            $message=$lang['flottealpha']['speznachricht6'];
        } elseif (($spezialmission==6) and ($betamodus==1)) {
            $spezialmission=26;
            $message=$lang['flottealpha']['speznachricht26'];
        } elseif ($spezialmission==7) {
            $message=$lang['flottealpha']['speznachricht7']; 
        } elseif ($spezialmission==8) { 
            $message=$lang['flottealpha']['speznachricht8']; 
        } elseif (($spezialmission==9) and ($betamodus<>1)) {
            $message=$lang['flottealpha']['speznachricht9'];
        } elseif (($spezialmission==9) and ($betamodus==1)) { 
            $spezialmission=10;
            $message=$lang['flottealpha']['speznachricht10'];
        } elseif ($spezialmission==11) {
            $message=$lang['flottealpha']['speznachricht11'];
        } elseif ($spezialmission==12) {
            $message=$lang['flottealpha']['speznachricht12'];
        } elseif ($spezialmission==13) {
            $message=$lang['flottealpha']['speznachricht13'];
        } elseif ($spezialmission==14) {
            $message=$lang['flottealpha']['speznachricht14'];
        } elseif ($spezialmission==15) {
            $message=$lang['flottealpha']['speznachricht15'];
        } elseif ($spezialmission==16) {
            $message=$lang['flottealpha']['speznachricht16'];
        } elseif (($spezialmission==17) and ($viralziel<>1)) { 
            $message=$lang['flottealpha']['speznachricht17'];
        } elseif (($spezialmission==17) and ($viralziel==1)) {
            $spezialmission=18;
            $message=$lang['flottealpha']['speznachricht18'];
        } elseif (($spezialmission==19) and ($betamodus<>1)) {
            $message=$lang['flottealpha']['speznachricht19'];
        } elseif (($spezialmission==19) and ($betamodus==1)) {
            $spezialmission=28;
            $message=$lang['flottealpha']['speznachricht28'];
        } elseif ($spezialmission==20) {
            $message=$lang['flottealpha']['speznachricht20'];
        } elseif ($spezialmission==21) {
            $traktor_id=intval($params["traktor_id"]);
            if ($traktor_id>=1) {
                $message=$lang['flottealpha']['speznachricht21t'];
                $Db->execute("UPDATE " . table_prefix . "schiffe set traktor_id='".$traktor_id."' where id='".$shid."'");
            } else {
                $spezialmission=0;
                $message=$lang['flottealpha']['speznachricht21f'];
            }
        } elseif ($spezialmission==22) {
            $message=$lang['flottealpha']['speznachricht22'];
        } elseif ($spezialmission==23) {
            $zeiger_extra = @mysql_query("SELECT extra FROM " . table_prefix . "schiffe where besitzer='".$spieler."' and id='".$shid."'");
            if(@mysql_num_rows($zeiger_extra) == 1) {
                ...
                $Db->execute("UPDATE " . table_prefix . "schiffe set extra='".$extra_neu."' where id='".$shid."'");
            }
            $message=$lang['flottealpha']['speznachricht23'];
            ...
            $message=$lang['flottealpha']['speznachricht24'];
        } elseif ($spezialmission==25) {
            $message=$lang['flottealpha']['speznachricht25'];
        } elseif ($spezialmission==29) {
            $message=$lang['flottealpha']['speznachricht29'];
        } elseif ($spezialmission==30) { 
            $spezialmission=30+intval($params["spielerid"]);
            if ($spezialmission==30){
                $message=$lang['flottealpha']['speznachricht30'];
            }else{
                $message=str_replace('{1}',$spielerfarbe[intval($params["spielerid"])],$lang['flottealpha']['speznachricht31']);
            }
        } elseif ($spezialmission==40) { 
            $spezialmission=40+intval($params["spielerfarbe"]);
            $message=str_replace('{1}',$spielerfarbe[intval($parans["spielerfarbe"])],$lang['flottealpha']['speznachricht41']);
        } elseif ($module[0] and $spezialmission == 51) {
            $zeiger_spionage = @mysql_query("SELECT extra FROM " . table_prefix . "schiffe where besitzer='".$spieler."' and id='".$shid."' and volk='unknown' and klasseid='1'");
            if(@mysql_num_rows($zeiger_spionage) == 1) {
                $spionage = @mysql_fetch_array($zeiger_spionage);
                $extra = @explode(":", $spionage['extra']);
                $extra_spio = @explode("-", $extra[0]);
                $extra_spio[3] = intval($params["spionage_id"]);
                $extra[0] = @implode("-", $extra_spio);
                $extra_neu = @implode(":", $extra);
                $Db->execute("UPDATE " . table_prefix . "schiffe set extra='".$extra_neu."' where id='".$shid."'");
            }
            $message = $lang['flottealpha']['speznachricht51'];
        } elseif ($spezialmission==61) {
            $spezialmission=intval($params["overdrive"]);
            $message=$lang['flottealpha']['speznachricht61'];
        } elseif ($spezialmission==70) {
            $message=$lang['flottealpha']['speznachricht70'];
        } elseif ($spezialmission==71) {
            $message=$lang['flottealpha']['speznachricht71'];
        } elseif ($spezialmission>71 and $spezialmission<77) {
            $zeiger = @mysql_query("SELECT * FROM " . table_prefix . "schiffe where besitzer='".$spieler."' and id='".$shid."'");
            $array = @mysql_fetch_array($zeiger);
            $masse=$array["masse"];
            $flug=$array["flug"];
            $fertigkeiten=$array["fertigkeiten"];
            ...
            if ($spezialmission==72){
                $message=$lang['flottealpha']['speznachricht72'];
            } elseif ($spezialmission>72 and $spezialmission<77){
                $message=str_replace('{1}',$spezialmission-71,$lang['flottealpha']['speznachricht73']);
            }
            if(($schalter==1) or ($masse>=100) or ($flug!=0)){
                $spezialmission=0;
                $lang['flottealpha']['speznachricht74'];
            }
        }
        $Db->execute("UPDATE " . table_prefix . "schiffe set spezialmission='".$spezialmission."' where id='".$shid."'");
        $zielid=@intval($params["schiff_id"]);
        $warp=@intval($params["warpfaktor"]);
        if (($begleitschutz==1) and (($flugalt!=4) or ($zielalt!=$zielid) or ($warpalt!=$warp))) {
            $flug=4;
            $zeiger = @mysql_query("SELECT id,kox,koy,name FROM " . table_prefix . "schiffe where id='".$zielid."'");
            $array = @mysql_fetch_array($zeiger);
            $zielx=$array["kox"];
            $ziely=$array["koy"];
            $name=$array["name"];
            ...
            if (($flugalt==4) and ($begleitschutz!=1)) {
                $Db->execute("update " . table_prefix . "schiffe set flug='0',warp='0',zielx='0',ziely='0',zielid='0' where id='".$shid."' and besitzer='".$spieler."'");
            }
            echo "<br>";
        }
        ?>
...
if ($fuid==5) {
    include (inhalt_dir . "inc.header.php");
    /*$zeiger = @mysql_query("SELECT id,routing_status,status,kox,koy FROM " . table_prefix . "schiffe where id='".$shid."'");
    $array = @mysql_fetch_array($zeiger);
    $routing_status=$array["routing_status"]; */
    $routing_status = $Db->getOne("SELECT routing_status FROM " . table_prefix . "schiffe where id='".$shid."'");
    //$array = @mysql_fetch_array($zeiger);
    //$routing_status=$array["routing_status"];
    ...
    <body text="#000000" background="<?php echo servername;?>bilder/aufbau/14.gif" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0"">
        <center>
            <table border="0" cellspacing="0" cellpadding="1">
                ...
                <tr>
                    ...
                    <td><a href="javascript:hilfe(30);"><img src="<?php echo servername;?>bilder/icons/hilfe.gif" border="0" width="17" height="17"></a></td>
                </tr>
            </table>
        </center>
        <br><br>
        ...
        <center>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td>
                        <form name="formular"  id="formular"  method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=6&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>"></td>
...
if ($fuid==6) {
    ...
    $routing_id="";
    $routing_koord="";
    if(urldecode(@$params["submitbutton"]) == html_entity_decode($lang['flottealpha']['uebernehmen'])){
        for($i=0;$i<intval($params["points"]);$i++){
            if($params["pid_"].$i!=-1){
                $routing_mins=$routing_mins.$params["cantox_"].$i.$params["vorrat_"].$i.$params["lem_"].$i.$params["bax_"].$i.$params["ren_"].$i.$params["vor_"].$i.$params["vol_"].$i;
                $temp_1=intval($params["kol_"].$i);
                for($j=1000000;$j>=1;$j=$j/10){
                    $temp_2=(int)($temp_1/$j);
                    $temp_1=$temp_1-($temp_2*$j);
                    $routing_mins=$routing_mins.$temp_2;
                }
                $temp_1=$params["lbt_"].$i;
                for($j=1000;$j>=1;$j=$j/10){
                    $temp_2=(int)($temp_1/$j);
                    $temp_1=$temp_1-($temp_2*$j);
                    $routing_mins=$routing_mins.$temp_2;
                }
                $temp_1=$params["sbt_"].$i;
                for($j=1000;$j>=1;$j=$j/10){
                    $temp_2=(int)($temp_1/$j);
                    $temp_1=$temp_1-($temp_2*$j);
                    $routing_mins=$routing_mins.$temp_2;
                ...
                $pid_a=intval($params["pid_"].$i);
                $zeiger = @mysql_query("SELECT x_pos,y_pos FROM " . table_prefix . "planeten where besitzer='".$spieler."' and id='".$pid_a."' and spiel='".$spiel."' order by name");
                ...
                if($i==0){
                    ...
                    $zielid=intval($params["pid_"].$i);
                }
                $routing_id=$routing_id.intval($params["pid_"].$i).":";
                $routing_koord=$routing_koord.$x_pos.":".$y_pos."::";
            }
        }
        ?>
        <body text="#000000" background="<?php echo servername;?>bilder/aufbau/14.gif" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0"">
        <form name="formular" id="formular"  method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=7&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>">
            <center><table border="0" cellspacing="0" cellpadding="5">
                <tr><td colspan="5"><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="1" height="5"></td></tr>
                <tr><td></td></tr>
                <tr><td></td></tr>
                <tr><td></td></tr>
                <tr>
        ...
        ?>
        <body text="#000000" background="<?php echo servername;?>bilder/aufbau/14.gif" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        <form name="formular" id="formular"  method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=6&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>">
            <center><table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td colspan="3"><img src="<?php echo servername;?>bilder/empty.gif" border="0" width="1" height="5"></td>
                </tr>
                <tr>
                    <?php
                    if(@$params["submitbutton"] == $lang['flottealpha']['neueroute']){
                        ?>
                        ...
                        <?php
                    }elseif(@$params["submitbutton"] == $lang['flottealpha']['routebearbeiten']){
                        $zeiger = @mysql_query("SELECT * FROM " . table_prefix . "schiffe where id='".$shid."'");
                        $array = @mysql_fetch_array($zeiger);
                    ...
                    }elseif(@$params["submitbutton"] == html_entity_decode($lang['flottealpha']['loeschen'])){
                        $point=@intval($params["point"]);
                        $points=@intval($params["points"])-1;
                        for($i=0;$i<$point;$i++){
                            $pid_h[$i]=@intval($params["pid_"].$i);
                            ?>
                            <input type="hidden" name="pid_<?php echo $i; ?>" value="<?php echo intval($params["pid_"].$i); ?>">
                            <input type="hidden" name="cantox_<?php echo $i; ?>" value="<?php echo intval($params["cantox_"].$i); ?>">
                            <input type="hidden" name="vorrat_<?php echo $i; ?>" value="<?php echo intval($params["vorrat_"].$i); ?>">
                            <input type="hidden" name="lem_<?php echo $i; ?>" value="<?php echo intval($params["lem_"].$i); ?>">
                            <input type="hidden" name="bax_<?php echo $i; ?>" value="<?php echo intval($params["bax_"].$i); ?>">
                            <input type="hidden" name="ren_<?php echo $i; ?>" value="<?php echo intval($params["ren_"].$i); ?>">
                            <input type="hidden" name="vor_<?php echo $i; ?>" value="<?php echo intval($params["vor_"].$i); ?>">
                            <input type="hidden" name="vol_<?php echo $i; ?>" value="<?php echo intval($params["vol_"].$i); ?>">
                            <input type="hidden" name="kol_<?php echo $i; ?>" value="<?php echo intval($params["kol_"].$i); ?>">
                            <input type="hidden" name="lbt_<?php echo $i; ?>" value="<?php echo intval($params["lbt_"].$i); ?>">
                            <input type="hidden" name="sbt_<?php echo $i; ?>" value="<?php echo intval($params["sbt_"].$i); ?>">
                            <?php
                        }
                        for($i=$point+1;$i<$points+1;$i++){
                            $pid_h[$i-1]=@intval($params["pid_"].$i);
                            ?>
                            <input type="hidden" name="pid_<?php echo $i-1; ?>" value="<?php echo intval($params["pid_"].$i); ?>">
                            <input type="hidden" name="cantox_<?php echo $i-1; ?>" value="<?php echo intval($params["cantox_"].$i); ?>">
                            <input type="hidden" name="vorrat_<?php echo $i-1; ?>" value="<?php echo intval($params["vorrat_"].$i); ?>">
                            <input type="hidden" name="lem_<?php echo $i-1; ?>" value="<?php echo intval($params["lem_"].$i); ?>">
                            <input type="hidden" name="bax_<?php echo $i-1; ?>" value="<?php echo intval($params["bax_"].$i); ?>">
                            <input type="hidden" name="ren_<?php echo $i-1; ?>" value="<?php echo intval($params["ren_"].$i); ?>">
                            <input type="hidden" name="vor_<?php echo $i-1; ?>" value="<?php echo intval($params["vor_"].$i); ?>">
                            <input type="hidden" name="vol_<?php echo $i-1; ?>" value="<?php echo intval($params["vol_"].$i); ?>">
                            <input type="hidden" name="kol_<?php echo $i-1; ?>" value="<?php echo intval($params["kol_"].$i); ?>">
                            <input type="hidden" name="lbt_<?php echo $i-1; ?>" value="<?php echo intval($params["lbt_"].$i); ?>">
                            <input type="hidden" name="sbt_<?php echo $i-1; ?>" value="<?php echo intval($params["sbt_"].$i); ?>">
                            <?php
                        }
                        $i=$point+1;
                        $i=($point==$points)?$point-1:$i;
                        $pid_a=intval($params["pid_"].$i);
                        $cantox_a=intval($params["cantox_"].$i);
                        $vorrat_a=intval($params["vorrat_"].$i);
                        $lem_a=intval($params["lem_"].$i);
                        $bax_a=intval($params["bax_"].$i);
                        $ren_a=intval($params["ren_"].$i);
                        $vor_a=intval($params["vor_"].$i);
                        $vol_a=intval($params["vol_"].$i);
                        $kol_a=intval($params["kol_"].$i);
                        $lbt_a=intval($params["lbt_"].$i);
                        $sbt_a=intval($params["sbt_"].$i);
                        $point=($point==$points)?$point-1:$point;
                        for($i=0;$i<$points;$i++){
                            $_POST["pid_".$i]=$pid_h[$i];
                        }
                    }elseif(@$params["submitbutton"] == html_entity_decode($lang['flottealpha']['neuerpunkt'])){
                        $point=intval($params["point"])+1;
                        $points=intval($params["points"])+1;
                        for($i=0;$i<$point;$i++){
                            $pid_h[$i]=intval($params["pid_"].$i);
                            ?>
                        ...
                        for($i=0;$i<$points;$i++){
                            ?>
                            <input type="hidden" name="pid_<?php echo $i+1; ?>" value="<?php echo intval($params["pid_"].$i); ?>">
                            <input type="hidden" name="kox_<?php echo $i; ?>" value="<?php echo intval($params["vvkox_"].$i); ?>">
                            <input type="hidden" name="koy_<?php echo $i; ?>" value="<?php echo intval($params["koy_"].$i); ?>">
                            <input type="hidden" name="cantox_<?php echo $i+1; ?>" value="<?php echo intval($params["cantox_"].$i); ?>">
                            <input type="hidden" name="vorrat_<?php echo $i+1; ?>" value="<?php echo intval($params["vorrat_"].$i); ?>">
                            <input type="hidden" name="lem_<?php echo $i+1; ?>" value="<?php echo intval($params["lem_"].$i); ?>">
                            <input type="hidden" name="bax_<?php echo $i+1; ?>" value="<?php echo intval($params["bax_"].$i); ?>">
                        ...
                        $pid_a=@intval($params["pid_"].$point);
                        $cantox_a=@intval($params["cantox_"].$point);
                        $vorrat_a=@intval($params["vorrat_"].$point);
                        $lem_a=@intval($params["lem_"].$point);
                        $bax_a=@intval($params["bax_"].$point);
                        $ren_a=@intval($params["ren_"].$point);
                        $vor_a=@intval($params["vor_"].$point);
                        $vol_a=@intval($params["vol_"].$point);
                        $kol_a=@intval($params["kol_"].$point);
                        $lbt_a=@intval($params["lbt_"].$point);
                        $sbt_a=@intval($params["sbt_"].$point);
                    }
                    if($points>1){
                    ...
                    if($params["wechsel"]==$lang['flottealpha']['fracht']){
                        ?>
                        ...
                        <td><select name="pid_<?php echo $point; ?>">
                            <?php
                            $zeiger = @mysql_query("SELECT id,besitzer,name FROM " . table_prefix . "planeten where besitzer='".$spieler."' and spiel='".$spiel."' order by name");
                            ...
                            if ($planetenanzahl>=1) {
                                for  ($i=0; $i<$planetenanzahl;$i++) {
                                    ...
                                    $piid=$array["id"];
                                    $name=$array["name"];
                                    if(($piid<>intval($params["pid_"].$davor))&&($piid<>intval($params["pid_"].$danach))&&($piid<>$pid_a))echo "<option value='".$piid."'>".$name."</option>";
                ...
                <center><table border="0" cellspacing="0" cellpadding="2">
                    ...
                    <tr>
                        <td><img src="<?php echo servername;?>bilder/icons/cantox.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['cantox']; ?>"></td>
                        ...
                        <td><img src="<?php echo servername;?>bilder/icons/lemin.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['lemin']; ?>"></td>
                        ...
                        <td><img src="<?php echo servername;?>bilder/icons/mineral_2.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['rennurbin']; ?>"></td>
                    ...
                    <tr>
                        <td><img src="<?php echo servername;?>bilder/icons/vorrat.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['vorrat']; ?>"></td>
                        ...
                        <td><img src="<?php echo servername;?>bilder/icons/mineral_1.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['baxterium']; ?>"></td>
                        ...
                        <td><img src="<?php echo servername;?>bilder/icons/mineral_3.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['vomisaan']; ?>"></td>
                ...
                <center>
                    <input type="submit" value="<" name="submitbutton">
                    	   <?php if($points>1){?><input type="submit" value="<?php echo $lang['flottealpha']['uebernehmen']; ?>" name="submitbutton"><?php } if(($points<10) and ($planetenanzahl>0)){?><input type="submit" value="<?php echo $lang['flottealpha']['neuerpunkt']; ?>"  name="submitbutton"><?php }if($points>1){?><input type="submit" value="<?php echo $lang['flottealpha']['loeschen']; ?>"  name="submitbutton"><?php }?><input type="hidden" value="<?php echo $lang['flottealpha']['fracht']; ?>" name="wechsel"><input type="submit" value="<?php echo $lang['flottealpha']['passagiere']; ?>" name="wechsel">
                    <input type="submit" value=">"  name="submitbutton" >
                </center>
                <?php
                ...
                <table border="0" cellspacing="0" cellpadding="1">
                    <tr>
                        ...
                        <td style="color:#aaaaaa;"><center>
                            ...
                            <?php
                            if($pid_a!=-1){
                                $zeiger = @mysql_query("SELECT id,besitzer,name FROM " . table_prefix . "planeten where besitzer='".$spieler."' and id='".$pid_a."' and spiel='".$spiel."' order by name");
                                $ok = @mysql_data_seek($zeiger,0);
                                $array = @mysql_fetch_array($zeiger);
                                $name=$array["name"];
                                echo $name;
                    ...
                    <tr>
                        <td><input type="hidden" name="point" value="<?php echo $point; ?>"><input type="hidden" name="points" value="<?php echo $points; ?>"><img src="<?php echo servername;?>bilder/icons/crew.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['kolonisten']; ?>"></td>
                        <td>
                    ...
                    <tr>
                        <td><img src="<?php echo servername;?>bilder/icons/leichtebt.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['leichtebt']; ?>"></td>
                        <td>
                    ...
                    <tr>
                        <td><img src="<?php echo servername;?>bilder/icons/schwerebt.gif" width="17" height="17" title="<?php echo $lang['flottealpha']['schwerebt']; ?>"></td>
                        <td>
...
if ($fuid==7) {
    include ("inc.header.php");
    $routing_mins=intval($params["routing_mins"]);
    $routing_id=intval($params["routing_id"]);
    $routing_koord=intval($params["routing_koord"]);
    $zielx=intval($params["('zielx"]);
    $ziely=intval($params["ziely"]);
    $zielid=intval($params["zielid"]);
    $flug=intval($params["flug"]);
    if ($params["submitbutton"]==$lang['flottealpha']['routeabschliessen']) {
        $zeiger = @mysql_query("SELECT * FROM " . table_prefix . "schiffe where id='".$shid."'");
        $array = @mysql_fetch_array($zeiger);
        $antrieb=$array["antrieb"];
        ...
        <body text="#000000" background="<?php echo servername;?>bilder/aufbau/14.gif" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
            <center>
                <table border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td><form name="formular" id="formular"  method="post" action="<?php echo servername;?>inhalt/flotte_alpha.php?fu=8&shid=<?php echo $shid; ?>&uid=<?php echo $uid; ?>&sid=<?php echo $sid; ?>"></td>
                        <td><?php echo $lang['flottealpha']['flugoptionen']; ?></td>
                        <td>
                            <input type="hidden" name="kox" value="<?php echo intval($params["kox"]); ?>">
                            <input type="hidden" name="koy" value="<?php echo intval($params["koy"]); ?>">
                            <input type="hidden" name="routing_mins" value="<?php echo $routing_mins?>">
                            <input type="hidden" name="routing_id" value="<?php echo $routing_id; ?>">
                            <input type="hidden" name="routing_koord" value="<?php echo $routing_koord; ?>">
                            <input type="hidden" name="zielx" value="<?php echo $zielx; ?>">
...
if ($fuid==8) {
    include (inhalt_dir . "inc.header.php");
    $Db->execute("UPDATE " . table_prefix . "schiffe set routing_id='" . $params["routing_id"] ."',
                                                                   routing_koord='" . $params["routing_koord"] . "',
                                                                   routing_mins='" . $params["routing_mins"] ."',
                                                                   routing_status='2',
                                                                   routing_schritt='0',
                                                                   routing_warp='" . intval($params["warpfaktor"]) . "',
                                                                   routing_tank='" . intval($params["tank"]) ."',
                                                                   routing_rohstoff='" . intval($params["rohstoff"]) . "' where id='".$shid."'");
    
    $Db->execute("UPDATE " . table_prefix . "schiffe set flug='" . intval($params["flug"]) . "',
                                                                        warp=".intval($params["warpfaktor"]) . "',
                                                                        plasmawarp='0',
                                                                        zielx='" . intval($params["zielx"]) . "',
                                                                        ziely='" . intval($params["ziely"]) . "',
    ...
    <body text="#000000" background="<?php echo servername;?>bilder/aufbau/14.gif" bgcolor="#000000" link="#000000" vlink="#000000" alink="#000000" leftmargin="0" rightmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        ...
        <script language="JavaScript">
            <?php
            if (( intval($params["kox"]) == intval($params["zielx"])) and (intval($params["koy"]) == intval($params["ziely"]))){
                ...
                $schrittx=(intval($params["zielx"])-intval($params["kox"]))/20;
                $schritty=(intval($params["ziely"])-iintval($params["koy"]))/20;               
                $laufx=intval($params["kox"]);
                $laufy=intval($params["koy"]);
                for ($k=1;$k<=20;$k++) {
                    $laufx=$laufx+$schrittx;
                    $laufy=$laufy+$schritty;
                    echo "parent.parent.mittemitte.document.getElementById('punkt_".$k."_".$shid."').style.left=".$laufx.";";
