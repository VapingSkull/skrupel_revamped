...
define('EXTENDDIR', $main_verzeichnis.'extend/'); 
$xstats_verzeichnis = $main_verzeichnis.'extend/xstats'; 

if (@intval(substr($spiel_extend,1,1))==1) {
    include(EXTENDDIR . "/ki/ki_basis/berechneKI.php");
}
*/
if (@intval(substr($spiel_extend,1,1))==1) {
    include(extend_dir . "ki/ki_basis/berechneKI.php");
}
if ((@file_exists(extend_dir . "xstats")) and (intval(substr($spiel_extend,2,1))==1)) {
    include(extend_dir.'xstats/xstatsCollect.php');
}
///////////////////////////////Sprachinclude(nur die benoetigten) Anfang
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "spiele WHERE id=$spiel");
$sprachtemp_1 = mysql_fetch_array($zeiger);
$sprachen = array();
for ($i=1; $i<=10; $i++){
    $spieler = 'spieler_'.$i;
    if($sprachtemp_1[$spieler] > 0) {
        $zeiger = mysql_query("SELECT * FROM " . table_prefix . "user WHERE id= '" . $sprachtemp_1[$spieler] . "' order by id");
...
mysql_query("DELETE FROM " . table_prefix . "kampf WHERE spiel='" . $spiel ."'");
mysql_query("DELETE FROM " . table_prefix . "nebel WHERE spiel='" . $spiel ."'");
mysql_query("DELETE FROM " . table_prefix . "scan WHERE spiel='" . $spiel ."'");
mysql_query("DELETE FROM " . table_prefix . "neuigkeiten WHERE sicher=0 AND spiel_id='" . $spiel . "' AND (art<=4 OR art=7 OR art=8)");
...
if ($module[4]==1) {
    $begegnungen = array();
    $zeiger = mysql_query("SELECT partei_a,partei_b FROM " . table_prefix . "begegnung where spiel='" . $spiel . "' order by spiel");
...
$zeiger = mysql_query("SELECT partei_a,partei_b,status,optionen FROM " . table_prefix . "politik WHERE spiel='" . $spiel . "' order by spiel");
...
///////////////////////////////////////////////////////////////////////////////////////////////POLITIKSTATUS ENDE
///////////////////////////////////////////////////////////////////////////////////////////////ROUTESTARTEN ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where flug='0' and status='2' and routing_status='2' and spiel='" . $spiel . "' order by id");
        ...
        if (isset($routing_koord) && $routing_koord!="")
        {
             if(($voll_laden!=1) or
                 ...
                 if(($voll_laden!=1) or 
                    ...
                    $zeigertemp = mysql_query("update " . table_prefix . "schiffe set flug='2',
                                                                                      warp='" . $warp . "',
                                                                                      zielx='" . $zielx . "',
                                                                                      ziely='" . $ziely ."',
                                                                                      zielid='" . $zielid . "',
                    ...
                    $zeigertemp = mysql_query("update " . table_prefix . "schiffe set flug='0',warp='0',zielx='0',ziely='0',zielid='0' where id='" . $shid . "'");
                }
            } else {
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][9],array($name));
                $zeigertemp = mysql_query("update " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0,routing_schritt=0,routing_koord='',routing_warp=0,routing_mins='',routing_id='',routing_tank=0,routing_status=0 where id='" . $shid . "'");
            }
        }
    }
}
...
if ($module[2]) {
    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0 where spiel='" . $spiel . "' and flug>=1 and (spezialmission=24 or spezialmission=25)");
}
///////////////////////////////////////////////////////////////////////////////////////////////MINENAKTION LOESCHEN BEI BEWEGUNG ENDE
///////////////////////////////////////////////////////////////////////////////////////////////TRAKTORSTRAHL UEBERPRUEFEN ANFANG
$zeiger = mysql_query("SELECT id,traktor_id,besitzer,warp FROM " . table_prefix . "schiffe where spezialmission=21 and spiel=$spiel order by id");
while($array = mysql_fetch_array($zeiger)) {
    list($shid, $traktor_id, $besitzer, $warp) = $array;
    if ($warp>7) {
        mysql_query("UPDATE " . table_prefix . "schiffe SET warp=7 WHERE id=$shid AND spiel=$spiel");
    }
    $zeiger2 = mysql_query("SELECT flug,besitzer,spezialmission FROM " . table_prefix . "schiffe WHERE id='" . $traktor_id ."' AND spiel='" . $spiel ."' order by id");
    $array2 = mysql_fetch_array($zeiger2);
    list($flug, $besitzer2, $spezialmission) = $array2;
    if ($flug>0 || $spezialmission>0 || $besitzer!=$besitzer2) {
        mysql_query("UPDATE " . table_prefix . "schiffe SET spezialmission='0',traktor_id='0' WHERE id='" . $shid . "' AND spiel='" . $spiel . "'");
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////TRAKTORSTRAHL UEBERPRUEFEN ENDE
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSUEBERGABE ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spezialmission>=31 and spezialmission<=40 and !(volk='unknown' and klasseid=1) and spiel='" . $spiel . "' order by id");
while($array = mysql_fetch_array($zeiger)) {
    $shid=$array['id'];
    $name=$array['name'];
    $volk=$array['volk'];
    ...
    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['uebergabe'][0],array($name,'<font color='.$spielerfarbe[$neu_besitzer].'>'.$neu_nick_besitzer.'</font>'));
    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$neu_besitzer,$lang['host'][$spielersprache[$neu_besitzer]]['uebergabe'][1],array('<font color='.$spielerfarbe[$besitzer].'>'.$nick_besitzer.'</font>',$name));
    mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission='0',besitzer='" . $neu_besitzer ."',fracht_leute='0',schwerebt='0',leichtebt='0',ordner='0' where id='" . $shid . "'");
}
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSUBERGABE ENDE
///////////////////////////////////////////////////////////////////////////////////////////////PLASMASTURM - SCHIFFE ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "anomalien where spiel='" . $spiel . "' and art='4' order by id");
...
if ($datensaetze>=1) {
    for  ($i=0; $i<$datensaetze;$i++) {
        ...
        $x_pos=$array["x_pos"];
        $y_pos=$array["y_pos"];
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0 where (spezialmission=13 or spezialmission=11 or spezialmission=12 or spezialmission=8 or spezialmission=7 or spezialmission=9 or spezialmission=10) and kox>='" . $x_pos . "' and kox<=" . $x_pos ."+10 and koy>=" . $y_pos . " and koy<=" . $y_pos . "+10 and zusatzmodul<>9 and spiel='" . $spiel . "'");
        $zeiger_temp = mysql_query("SELECT id,warp,plasmawarp FROM " . table_prefix . "schiffe where warp>5 and kox>=" . $x_pos . " and kox<=" . $x_pos . "+10 and koy>=" . $y_pos ." and koy<=" . $y_pos . "+10 and zusatzmodul<>9 and spiel='" . $spiel . "' order by id");
        ...
        if ($datensaetze_temp>=1) {
            for  ($j=0; $j<$datensaetze_temp;$j++) {
                ...
                mysql_query("UPDATE " . table_prefix . "schiffe set plasmawarp='" . $plasmawarp . "',warp='5' where spiel='" . $spiel . "' and id='" . $shid . "'");
            }
        }
    }
}
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spiel='" . $spiel . "' and plasmawarp<>0 order by id");
$datensaetze = mysql_num_rows($zeiger);
if ($datensaetze>=1) {
    for  ($i=0; $i<$datensaetze;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "anomalien where art='4' and x_pos<=" . $kox . " and x_pos>=" . $kox . "-10 and y_pos<=". $koy ." and y_pos>=" . $koy ."-10 and spiel='" . $spiel . "' order by id");
        $datensaetze_temp = mysql_num_rows($zeiger_temp);
        if ($datensaetze_temp>=1){
        } else {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set warp=$plasmawarp, plasmawarp=0 where spiel=$spiel and id=$shid");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////PLASMASTURM - SCHIFFE ENDE
///////////////////////////////////////////////////////////////////////////////////////////////BODENKAMPF ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "planeten where kolonisten_spieler>=1 and ((kolonisten_new>=1) or (leichtebt_new>=1) or (schwerebt_new>=1)) and besitzer>=1 and spiel='" . $spiel . "' order by id");
$planetenanzahl = mysql_num_rows($zeiger);
if ($planetenanzahl>0) {
    include(inhalt_dir.'inc.host_bodenkampf.php');
}
///////////////////////////////////////////////////////////////////////////////////////////////BODENKAMPF ENDE
///////////////////////////////////////////////////////////////////////////////////////////////WELLENGENERATOR ANFANG
$reichweite = 65;
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe WHERE spezialmission=70 and spiel='" . $spiel . "' order by id");
while($array = mysql_fetch_array($zeiger)) {
    $shid = $array["id"];
    $name = $array["name"];
    $kox = $array["kox"];
    ...
    if ($wellengenerator_fert>=1 && $vomisaan >= $wellengenerator_fert) {
        $erfolg = false;
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe WHERE (sqrt(((kox-" . $kox .")*(kox-" . $kox . "))+((koy-".$koy.")*(koy-".$koy.")))<=".$reichweite.") and spiel='" . $spiel . "' order by id");
        while($array_temp = mysql_fetch_array($zeiger_temp)) {
            ...
            if($lichtjahre2 <= $reichweite*$reichweite) {
                if(($t_spezialmission!=7 && $t_spezialmission!=16) || $t_besitzer==$besitzer || ($beziehung[$besitzer][$t_besitzer]['status']>=3 && $beziehung[$besitzer][$t_besitzer]['status']<=5)) {
                    if($t_warp > 7) {
                        neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['wellengenerator'][2],array($t_name));
                        mysql_query("UPDATE " . table_prefix . "schiffe set warp=7 where id='" . $t_shid ."'");
                    }
                } else {
                    $erfolg = true;
                    neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['wellengenerator'][0],array($t_name));
                    mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,warp=0,flug=0,zielx=0,ziely=0,zielid=0 where id='" . $t_shid . "'");
                }
            }
        }
        $vomisaan -= $wellengenerator_fert;
        mysql_query("UPDATE " . table_prefix . "schiffe set fracht_min3='" . $vomisaan . "' where id='" . $shid . "'");
        if ($erfolg) {
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wellengenerator'][3],array($name));
        }
    } else {
        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wellengenerator'][1],array($name));
        mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0 where id='" . $shid . "'");
    }
}

/*
 * Bis hierhin sind die Datenbankabfragen korrigiert und angepaßt. *Skull*
 */
///////////////////////////////////////////////////////////////////////////////////////////////WELLENGENERATOR ENDE
///////////////////////////////////////////////////////////////////////////////////////////////SPRUNGTRIEBWERK ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where flug>=1 and flug<=2 and status>0 and spezialmission=7 and spiel='" . $spiel . "'order by id");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        if ($lemin>=$fert_sprung_kosten) {
            ...
            $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe WHERE kox>=" . $rand_x_a . " and kox<=" . $rand_x_b . " and koy>=" . $rand_y_a . " and koy<=" . $rand_y_b . " and spezialmission=70 and spiel='" . $spiel . "' order by kox ".($kox<$kox_neu?"asc":"desc").", koy ".($koy<$koy_neu?"asc":"desc"));
            while($array_temp = mysql_fetch_array($zeiger_temp)) {
                ...
                if ($t_besitzer != $besitzer && $beziehung[$besitzer][$t_besitzer]['status'] < 3) {
                    ...
                    if ($a_ <= 65) {
                        ...
                        neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['wellengenerator'][4],array($t_name));
                        neuigkeiten(2,servername . "bilder/news/sprung.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtriebwerk'][3],array($name,(int)$reichweite2));
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,kox=$kox_neu, koy=$koy_neu, lemin=$lemin, flug=0, status=1 where id=$shid");
                        continue(2);
                    }
                }
            }
            if (($kox_neu>=10) and ($kox_neu<=$umfang-13) and ($koy_neu>=10) and ($koy_neu<=$umfang-13)) {
                neuigkeiten(2, servername . "bilder/news/sprung.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtriebwerk'][0],array($name,$reichweite));
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,kox='" . $kox_neu . "', koy='" . $koy_neu ."', lemin='" . $lemin . "', flug=0, status=1 where id='" . $shid . "'");
            } else {
                $schiffverschollen++;
                neuigkeiten(2, servername . "bilder/news/sprung.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtriebwerk'][1],array($name));
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "schiffe where id='" . $shid . "'");
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:".$shid.":%'");
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where flug=3 and zielid='" . $shid . "'");
            }
        } else {
            neuigkeiten(2,servername . "bilder/news/sprung.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtriebwerk'][2],array($name));
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,flug=0 where id='" . $shid . "'");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////SPRUNGTRIEBWERK ANFANG
///////////////////////////////////////////////////////////////////////////////////////////////SUBRAUMVERZERRUNG ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spezialmission=9 and spiel='" . $spiel . "' order by id");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where (sqrt((($kox-kox)*($kox-kox))+(($koy-koy)*($koy-koy)))<=83) and spezialmission<>9 and spiel='" . $spiel . "' order by id");
        ...
        if ($treffschiff>=1) {
            for ($k=0; $k<$treffschiff;$k++) {
                ...
                if ($schaden<100) {
                    neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['subraumverzerrer'][0],array($t_name,$schaden));
                    $zeiger_temp2 = mysql_query("UPDATE" . table_prefix . "schiffe set schaden='" . $schaden . "' where id='" . $t_shid . "'");
                }
                if ($schaden>=100) {
                    neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['subraumverzerrer'][1],array($t_name));
                    $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id='" . $t_shid . "'");
                    $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:" . $t_shid . ":%'");
                    $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid='" .  $t_shid . "'");
                }
            }
        }
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "anomalien where (sqrt(($kox-x_pos)*($kox-x_pos)+($koy-y_pos)*($koy-y_pos))<=83) and art=3 and spiel='" . $spiel . "' order by id");
        ...
        if ($trefffalte>=1) {
            for ($k=0; $k<$trefffalte;$k++) {
                ...
                $war=mt_rand(1,10);
                if($war<=$fert_subver){
                    $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where id='" . $fid . "'");
                }
            }
        }
        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['subraumverzerrer'][2],array($name));
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id='" . $shid . "'");
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:" . $shid . ":%'");
        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid='" . $shid . "'");
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////SUBRAUMVERZERRUNG ENDE
///////////////////////////////////////////////////////////////////////////////////////////////LOYDS FLUCHTMANOEVER ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spezialmission=16 and spiel='" . $spiel . "' order by spiel");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $koy=$s_y;
        if ($fluchtmanoever==1) {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,kox='" . $kox . "' ,koy='" . $koy . "',flug=0,warp=0,zielx=0,ziely=0,zielid=0 where id='" . $shid . "'");
            $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid='" . $shid . "'");
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['fluchtmanoever'][0],array($name));
        }
        if ($fluchtmanoever>=2) {
            $schadenbumm=mt_rand(1,$fluchtmanoever);
            $schaden=$schaden+$schadenbumm;
            if ($schaden<100) {
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['fluchtmanoever'][1],array($name,$schadenbumm));
                $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid='" . $shid . "'");
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set schaden='" . $schaden . "',spezialmission=0,kox='" . $kox . "',koy='" . $koy . "',flug=0,warp=0,zielx=0,ziely=0,zielid=0 where id='" . $shid . "'");
            }
            if ($schaden>=100) {
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['fluchtmanoever'][2],array($name));
                $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id='" . $shid ."");
                $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "sanomalien where art=3 and extra like 's:".$shid.":%'");
                $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug='0',warp='0',zielx='0',ziely='0',zielid='0' where (flug='3' or flug='4') and zielid='" . $shid ."'");
            }
        }
    }
...
mysql_query("UPDATE " . table_prefix . "schiffe SET kox_old='0',koy_old='0',temp_verfolgt='0' WHERE spiel='" . $spiel . "'");
//Verfolger auf 1 setzen
mysql_query("UPDATE " . table_prefix . "schiffe SET temp_verfolgt='1' WHERE spiel='" . $spiel . "' AND flug>2");
//setze temp_verfolgt auf 1 bei allen Schiffen die verfolgt werden
$zeiger = mysql_query("SELECT DISTINCT zielid FROM " . table_prefix . "schiffe where flug>2 and spiel='" . $spiel . "'");
while($array = mysql_fetch_array($zeiger)) {
    $zid = $array['zielid'];
    mysql_query("UPDATE " . table_prefix . "schiffe SET temp_verfolgt=1 WHERE spiel='" . $spiel . "' AND id='" . $zid . "'");
}
$zeiger = mysql_query("SELECT id,zielid,flug,kox,koy FROM " . table_prefix . "schiffe WHERE flug>0 AND status>0 AND spiel='" . $spiel . "' AND temp_verfolgt='1' ORDER BY zielid DESC");
$schiffanzahl = mysql_num_rows($zeiger);
if($schiffanzahl>0){
    for  ($i=0; $i< $schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
...
if($schiffanzahl>0){
    do{
        ...
        if($schalter){
            ...
            for($k=0;$k< $anzahl_schlange;$k++){
                $j=array_search($feld_zielid[$j],$feld_shid);
                $zeiger3 = mysql_query("SELECT warp FROM " . table_prefix . "schiffe where id='" . $feld_shid[$j] . "' and spiel='" . $spiel . "'");
            ...
            $zeiger2 = mysql_query("UPDATE " . table_prefix . "schiffe set zielx='" . $MP_x . "',ziely='" . $MP_y . "',zielid=-1 where spiel='" . $spiel . "' and id='" . $feld_shid[$feldindex_maxentfernung] . "'");
...
//e
for($i=0; $i<$schiffanzahl;$i++) {
    $zeiger = mysql_query("UPDATE " . table_prefix . "schiffe set temp_verfolgt='" . $feld_schlange[$i] . "' where spiel='" . $spiel . "' and id='" . $feld_shid[$i] . "'");
}
//so wir haben es geschaft alles ist wohlgeordnet jetzt kann geflogen werden
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where flug>0 and status>0 and spiel='" . $spiel . "' order by temp_verfolgt");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        if ($spezialmission==21) {
            $zeiger2 = mysql_query("SELECT id,masse,spiel FROM " . table_prefix . "schiffe where id='" . $traktor_id . "' and spiel='" . $spiel . "'");
            ...
            if ($trakanzahl>=1) {
                ...
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission='0',traktor_id='0' where id='" . $shid . "' and spiel='" . $spiel . "'");
            }
        }
        ////////////////////////////overdrive
        $overdrive=0;
        ...
        if (($spezialmission>=61) and ($spezialmission<=69)) {
            ...
            if ($temp<=($overdrive_stufe*10)) {
                $overdrive_raus=1;
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][0],array($name));
            } else {
                $overdrive=1;
        ...
        if ((($flug==4) or ($flug==3))and ($zielid!=-1)) {
            $zeiger_temp = mysql_query("SELECT id,kox,koy,spezialmission,antrieb,tarnfeld,besitzer,name,bild_gross,volk FROM " . table_prefix . "schiffe where id='".$zielid."' order by id");
            $array_temp = mysql_fetch_array($zeiger_temp);
            $zielxt=$zielx;
            $zielyt=$ziely;
    ...
    if((($zielx-$kox)*($zielx-$kox))+(($ziely-$koy)*($ziely-$koy))<=($scan_temp_reichweite*$scan_temp_reichweite)){
      $n_gescannt=0;
                ...
                $zeiger_temp2 = mysql_query("SELECT besitzer FROM " . table_prefix . "schiffe where (
                ...
                $zeiger_temp2 = mysql_query("SELECT besitzer FROM " . table_prefix . "planeten where (
                ...
                if($n_gescannt==1){
                    ...
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][7],array($spielerfarbe[$besitzer],$name,$sektork,$spielerfarbe[$besitzer_2],$name_2));
                    neuigkeiten(2,servername . "daten/$volk_2/bilder_schiffe/$bild_gross_2",$besitzer_2,$lang['host'][$spielersprache[$besitzer_2]]['flug'][8],array($spielerfarbe[$besitzer_2],$name_2,$sektork,$spielerfarbe[$besitzer],$name));
                }
            }
        }
        if ((($kox!=$zielx) or ($koy!=$ziely)) and ($overdrive_raus==0)) {
            ...
            if ($zufall<=11) {
                $zeit=$lichtjahre/(9*9);
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][1],array($name));
            }
        }
        ...
        if (($antrieb==4) and ($verbrauch>=1)) {
            ...
            if ($zufall<=17) {
                ...
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][2],array($name,$verbrauch,$verbrauchneu));
        ...
        if ($verbrauch>$lemin) { $rauswurf=2; } else {
            ...
            if ($zeit<=1) {
                ...
                if ($flug==1) {
                    $flug_neu=0;
                    $status=1;
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][3],array($name));
                }
                if ($flug==2) {
                    $flug_neu=0;
                    $status=2;
                    if ($routing_status>=1) {
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][4],array($name));
                    } else {
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][5],array($name));
                    }
                }
                if ($flug==4) {
                    $flug_neu=$flug;
        ...
        if ($rauswurf==1) {
            if (($flug==1) or ($flug==2)) {
                $zeiger2 = mysql_query("UPDATE " . table_prefix . "schiffe set kox_old='" . $kox_old . "',koy_old='" . $koy_old . "',strecke=strecke+".$streckemehr.",fracht_min2='" . $fracht_min2 . "',kox='" . $kox . "', koy='" . $koy . "', lemin='" . $lemin . "', flug='" . $flug_neu . "', status='" . $status  . "' where id='" . $shid . "'");
            }
            if (($flug==4) or ($flug==3)) {
                $zeiger2 = mysql_query("UPDATE " . table_prefix . "schiffe set kox_old='" . $kox_old . "',koy_old='" . $koy_old . "',strecke=strecke+" . $streckemehr . ",fracht_min2='" . $fracht_min2 . "',kox='" . $kox . "', koy='" . $koy . "', zielx='" . $zielx . "', ziely='" . $ziely . "', lemin='" . $lemin . "', flug='" . $flug_neu . "', status='" . $status . "' where id='" . $shid . "'");
            }
            $stat_lichtjahre[$besitzer]=$stat_lichtjahre[$besitzer]+$streckemehr;
            if ($spezialmission==21) {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set kox='" . $kox . "', koy='" . $koy . "', status='" . $status  . "' where id='" . $traktor_id  . "' and spiel='" . $spiel . "'");
            }
        } else {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug='0',warp='0',zielx='0',ziely='0',zielid='0' where id='" . $shid . "'");
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set routing_schritt=0,routing_status=0,routing_koord='',routing_id='',routing_mins='',routing_warp=0,routing_tank=0,routing_rohstoff=0 where id=$shid");
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][6],array($name));
        }
    }elseif(($overdrive_raus==1)and(($flug==4) or ($flug==3))and ($zielid!=-1)){
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set zielx=$zielx,ziely=$ziely where id=$shid");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////FLUG ENDE
...
if($kreisel_anzahl>0){
    for($i=0;$i< $kreisel_anzahl;$i++){
        $zeiger2 = mysql_query("SELECT kox,koy FROM " . table_prefix . "schiffe where spiel=$spiel and id=$zwischenarray_zielid[$i]");
        $array2 = mysql_fetch_array($zeiger2);
        $t_kox=$array2["kox"];
        $t_koy=$array2["koy"];
        $zeiger2 = mysql_query("UPDATE " . table_prefix . "schiffe set zielx=$t_kox,ziely=$t_koy,zielid=$zwischenarray_zielid[$i] where spiel=$spiel and id=$zwischenarray_shid[$i]");
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////ZIELKORREKTUR ENDE
///////////////////////////////////////////////////////////////////////////////////////////////ERFAHRUNG DURCH STRECKE ANFANG
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set erfahrung=erfahrung+1,strecke=strecke-1000 where strecke>999 and erfahrung<5 and spiel=$spiel");
///////////////////////////////////////////////////////////////////////////////////////////////ERFAHRUNG DURCH STRECKE ENDE
///////////////////////////////////////////////////////////////////////////////////////////////WURMLOCH ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "anomalien where spiel=$spiel order by id");
$datensaetze = mysql_num_rows($zeiger);
if ($datensaetze>=1) {
    for  ($i=0; $i<$datensaetze;$i++) {
    $ok = mysql_data_seek($zeiger,$i);
        ...
        if (($art==1) or ($art==2)) {
            ...
            $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where sqrt( (kox-$x_pos)*(kox-$x_pos)+(koy-$y_pos)*(koy-$y_pos) )<=$reichweite and spiel=$spiel order by id");
                ...
                for  ($k=0; $k<$schiffanzahl;$k++) {
                    ...
                    if ($extras[0]>=1) {
                        if ($spezialmission==29) {
                            $zeiger2 = mysql_query("UPDATE " . table_prefix . "anomalien set extra='' where id=$aid");
                            $aid2=intval($extras[0]);
                            $zeiger2 = mysql_query("UPDATE " . table_prefix . "anomalien set extra='' where id=$aid2");
                        }
                        ...
                        $zeiger2 = mysql_query("UPDATE " . table_prefix . "schiffe set kox=$x, koy=$y, zielx=0, ziely=0, flug=0, status=1  where id=$shid");
                        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
                        if ($art==1) {
                            if ($spezialmission==29) {
                                neuigkeiten(2,servername . "bilder/news/wurmloch.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][4],array($name));
                            } else {
                                neuigkeiten(2,servername . "bilder/news/wurmloch.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][0],array($name));
                            }
                        }elseif ($art==2) {
                            if ($spezialmission==29) {
                                neuigkeiten(2,servername . "bilder/news/sprungtor.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][5],array($name));
                            } else {
                                neuigkeiten(2,servername . "bilder/news/sprungtor.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][1],array($name));
                            }
                        }
                    } else {
                        $ok=1;
                        while ($ok==1) {
                            ...
                            $ok=2;
                            $nachbarn=0;
                            $zeiger2 = mysql_query("SELECT count(*) as total from " . table_prefix . "planeten where sqrt( (x_pos-$x)*(x_pos-$x)+(x_pos-$x)*(x_pos-$x) )<=20 and spiel=$spiel");
                        ...
                        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
                        if ($art==1) {
                            neuigkeiten(2,servername . "bilder/news/wurmloch.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][2],array($name));
                        }elseif ($art==2) {
                            neuigkeiten(2,servername . "bilder/news/sprungtor.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wurmloch'][3],array($name));
                        }
                    }
                }
            }
...
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spezialmission=30 and spiel=$spiel order by id");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where (sqrt(((kox-$kox)*(kox-$kox))+((koy-$koy)*(koy-$koy)))<=$reichweite) and tarnfeld=1 and spiel=$spiel order by id");
        $treffschiff = mysql_num_rows($zeiger_temp);
        if ($treffschiff>=1) {
            for ($k=0; $k<$treffschiff;$k++) {
                $ok2 = mysql_data_seek($zeiger_temp,$k);
                ...
                $t_besitzer=$array_temp["besitzer"];
                $t_bild_gross=$array_temp["bild_gross"];
                neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugunverzerrer'][0],array($t_name));
                $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where id=$t_shid");
            }
        }
        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['drugunverzerrer'][1],array($name));
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id=$shid");
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:$shid:%'");
        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////SELFDESTRUCT ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where spezialmission=15 and spiel=$spiel order by id");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $reichweite=83;
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where (sqrt(((kox-$kox)*(kox-$kox))+((koy-$koy)*(koy-$koy)))<=$reichweite) and spezialmission<>15 and spiel=$spiel order by id");
        $treffschiff = mysql_num_rows($zeiger_temp);
        if ($treffschiff>=1) {
            for ($k=0; $k<$treffschiff;$k++) {
                $ok2 = mysql_data_seek($zeiger_temp,$k);
                ...
                $schaden=round($t_schaden+($sub_schaden*(80/($t_masse+1))*(80/($t_masse+1))+2));
                if ($schaden<100) {
                    neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['selfdestruct'][0],array($t_name,$schaden));
                    $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set schaden=$schaden where id=$t_shid");
                }
                if ($schaden>=100) {
                    neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['selfdestruct'][1],array($t_name));
                    $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id=$t_shid");
                    $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:$t_shid:%'");
                    $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$t_shid");
                }
        ...
        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['selfdestruct'][2],array($name));
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "schiffe where id=$shid");
        $zeiger_temp2 = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:$shid:%'");
        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////SELFDESTRUCT ENDE
///////////////////////////////////////////////////////////////////////////////////////////////RAUMFALTEN ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "anomalien where extra like 's:%' and art=3 and spiel=$spiel order by id");
...
if ($anoanzahl>=1) {
    for  ($i=0; $i<$anoanzahl;$i++) {
        ...
        $extra=$array["extra"];
        $extras=explode(":",$extra);
        $zeiger_temp = mysql_query("SELECT id,kox,koy,spiel FROM " . table_prefix . "schiffe where  spiel=$spiel and id=".$extras[1]." order by id");
        $array_temp = mysql_fetch_array($zeiger_temp);
        $kox=$array_temp["kox"];
        $koy=$array_temp["koy"];
        $optionen="s:".$extras[1].":$kox:$koy:".$extras[4].":".$extras[5].":".$extras[6].":".$extras[7].":".$extras[8].":".$extras[9];
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set extra='$optionen' where spiel=$spiel and id=$anoid");
    }
}
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "anomalien where  art=3 and spiel=$spiel order by id");
$anoanzahl = mysql_num_rows($zeiger);
if ($anoanzahl>=1) {
    for  ($i=0; $i<$anoanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        if ($zeit<=1) {
            if ($extras[0]=='p') {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set cantox=cantox+".$extras[4].", vorrat=vorrat+".$extras[5].", lemin=lemin+".$extras[6].", min1=min1+".$extras[7].", min2=min2+".$extras[8].", min3=min3+".$extras[9]." where id=".$extras[1]." and spiel=$spiel");
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien  where id=$anoid and spiel=$spiel");
                $zeiger_temp = mysql_query("SELECT id,besitzer,name FROM " . table_prefix . "planeten where spiel=$spiel and id=".$extras[1]);
                $array_temp = mysql_fetch_array($zeiger_temp);
                $name=$array_temp["name"];
                $besitzer=$array_temp["besitzer"];
                if ($besitzer>=1)  {
                    neuigkeiten(1,servername . "bilder/news/raumfalte.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['raumfalte'][0],array($name,$extras[4],$extras[6],$extras[8],$extras[5],$extras[7],$extras[9]));
                }
            }elseif ($extras[0]=='s') {
                $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where id=".$extras[1]." and spiel=$spiel");
                $array_temp = mysql_fetch_array($zeiger_temp);
                $besitzer=$array_temp["besitzer"];
                ...
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set lemin=$fracht_lemin,fracht_vorrat=$fracht_vorrat,fracht_cantox=$fracht_cantox,fracht_min1=$fracht_min1,fracht_min2=$fracht_min2,fracht_min3=$fracht_min3 where id=".$extras[1]." and spiel=$spiel");
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien  where id=$anoid and spiel=$spiel");
                neuigkeiten(2,servername . "bilder/news/raumfalte.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['raumfalte'][1],array($name,$extras[4],$extras[6],$extras[8],$extras[5],$extras[7],$extras[9]));
            }
        } else {
            $kox=round($kox+(($zielx-$kox)/$zeit));
            $koy=round($koy+(($ziely-$koy)/$zeit));
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set x_pos=$kox, y_pos=$koy where id=$anoid");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////RAUMFALTEN ENDE
///////////////////////////////////////////////////////////////////////////////////////////////AUTOPROJEKTILE ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where projektile_anzahl>=1 and projektile_auto=1 and spiel=$spiel");
$anoanzahl = mysql_num_rows($zeiger);
if ($anoanzahl>=1) {
    for  ($i=0; $i<$anoanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        if ($max_bau>=1) {
            ...
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set projektile=$projektile,fracht_cantox=$fracht_cantox,fracht_min1=$fracht_min1,fracht_min2=$fracht_min2 where id=$shid");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////AUTOPROJEKTILE ENDE
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSBAU ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "sternenbasen where schiffbau_status=1 and status=1 and spiel=$spiel order by id");
...
if ($basenanzahl>=1) {
    ...
    for  ($i=0; $i<$basenanzahl;$i++) {
        ...
        $x_pos=$array["x_pos"];
        $y_pos=$array["y_pos"];
        $zeiger2 = mysql_query("SELECT * FROM " . table_prefix . "planeten where x_pos=$x_pos and y_pos=$y_pos and spiel=$spiel order by id");
        ...
        if($schalter==1){
            ...
            if($schalter==1){
                $zeiger3 = mysql_query("SELECT * FROM " . table_prefix . "huellen where baid=$baid and spiel=$spiel and klasse=$schiffbau_klasse order by id");
        ...
        neuigkeiten(2,servername . "daten/$schiffbau_rasse/bilder_schiffe/$schiffbau_bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['schiffbau'][0],array($schiffbau_name));
    }
}
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "sternenbasen set schiffbau_status=0,schiffbau_extra='' where spiel=$spiel");
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSBAU ENDE
///////////////////////////////////////////////////////////////////////////////////////////////GRAVITATION ANFANG
    ...
    for  ($ir=0; $ir<$schiffanzahl;$ir++) {
        ...
        if ($planetenanzahl>=1) {
            for  ($i=0; $i<$planetenanzahl;$i++) {
                ...
                if ($pid==$zielid) {
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['flug'][5],array($name));
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0 where id=$shid");
                }
...
if($module[2]) {
    ...
    if ($schiffanzahl>=1) {
        for  ($i=0; $i<$schiffanzahl;$i++) {
            ...
            //echo "SELECT * FROM " . table_prefix . "anomalien where spiel=$spiel and art=5 and x_pos>=$rand_x_a and x_pos<=$rand_x_b and y_pos>=$rand_y_a and y_pos<=$rand_y_b order by id"."<br><br>";
            ...
            if ($minenanzahl>=1) {
                ...
                if ($minentreffer>=1) {
                    ...
                    if ($aanomalie[2]<=0) {
                        $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where spiel=$spiel and id=$aanomalie[0]");
                    } else {
                        $mineextra=$aanomalie[1].':'.$aanomalie[2].':'.$aanomalie[3];
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set extra='$mineextra' where spiel=$spiel and id=$aanomalie[0]");
                    }
                    ...
                    if (($schaden>=100) or ($crew<1)) {
                        ...
                        $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:$shid:%'");
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][0],array($name,$sektork));
                    } else {
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set crew=$crew,schaden=$schaden,scanner=0 where id=$shid and besitzer=$besitzer;");
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][1],array($name,$sektork,$minentreffer,$schaden_rumpf,$schaden_crewmen));
                    }
                }
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////SPIONAGE ANFANG
if($module[0]) {
    include(inhalt_dir .'inc.host_spionage.php');
}
///////////////////////////////////////////////////////////////////////////////////////////////SPIONAGE ENDE
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        $fert_subver=intval(substr($fertigkeiten,23,1));
        $sub_schaden=$fert_subver*50;
        $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "schiffe where (sqrt(($kox-kox)*($kox-kox)+($koy-koy)*($koy-koy))<=83) and spezialmission<>10 and spiel=$spiel order by id");
        $treffschiff = mysql_num_rows($zeiger_temp);
        if ($treffschiff>=1) {
            for ($k=0; $k<$treffschiff;$k++) {
                $ok2 = mysql_data_seek($zeiger_temp,$k);
...
include(inhalt_dir . 'inc.host_orbitalkampf.php');
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSKAMPF PLANET ENDE
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSKAMPF ANFANG
include(inhalt_dir . 'inc.host_raumkampf.php');
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSKAMPF ENDE
///////////////////////////////////////////////////////////////////////////////////////////////STERNENBASEN ANFANG
///////////////////////////////////////////////////////////////////////////////////////////////STERNENBASEN BAUEN ANFANG
$zeiger = mysql_query("SELECT * FROM " . table_prefix . "sternenbasen where status=0 and spiel=$spiel order by id");
$basenanzahl = mysql_num_rows($zeiger);
if ($basenanzahl>=1) {
    for ($i=0; $i<$basenanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        $planetid=$array["planetid"];
        $besitzer=$array["besitzer"];
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "sternenbasen set status=1 where id=$bid");
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set sternenbasis=2 where id=$planetid");
        $neuebasen++;
        neuigkeiten(3,servername . "daten/$rasse/bilder_basen/1.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['basenbauen'][0],array($name));
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////STERNENBASEN BAUEN ENDE
///////////////////////////////////////////////////////////////////////////////////////////////STERNENBASEN ENDE
///////////////////////////////////////////////////////////////////////////////////////////////SCHIFF WEICHT PLANET AUS ANFANG
$zeiger = mysql_query("SELECT id,status,besitzer,name,klasse,klasseid,kox,koy,volk,bild_gross FROM " . table_prefix . "schiffe where status=2 and spiel=$spiel order by id");
$schiffanzahl = mysql_num_rows($zeiger);
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
        ...
        if ($gemeinsam>=1) {
            $zeiger2 = mysql_query("SELECT x_pos,y_pos,spiel,id,name,besitzer,bild,klasse FROM " . table_prefix . "planeten where x_pos=$kox and y_pos=$koy and spiel=$spiel");
            $array2 = mysql_fetch_array($zeiger2);
            $p_id=$array2["id"];
            $p_name=$array2["name"];
            ...
            if (($beziehung[$besitzer][$p_besitzer]['status']==3) or ($beziehung[$besitzer][$p_besitzer]['status']==4)) {
                ...
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set status=1,kox=$kox, koy=$koy where id=$shid and spiel=$spiel");
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['ausweichen'][0],array($name,$spielerfarbe[$p_besitzer],$p_name));
            }
        }
    }
}
            ...
            if ($gemeinsam>=1) {
                ...
                for ($ihj=0; $ihj<$gemeinsam;$ihj++) {
                    ...
                    if (strstr($checkstring,$code)) {} else {
                        ...
                        if ($springer==1) {
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set status=1,kox=$kox, koy=$koy where id=$shid and spiel=$spiel");
                            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['ausweichen'][1],array($name,$spielerfarbe[$besitzer_2],$name_2));
                            $checkstring=$checkstring.":::".$shid.":::";
                        } else {
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set status=1,kox=$kox, koy=$koy where id=$shid_2 and spiel=$spiel");
                            neuigkeiten(2,servername . "daten/$volk_2/bilder_schiffe/$bild_gross_2",$besitzer_2,$lang['host'][$spielersprache[$besitzer_2]]['ausweichen'][1],array($name_2,$spielerfarbe[$besitzer],$name));
    ...
    for ($i=0; $i<$schiffanzahl;$i++) {
            ...
            if($status!=2){
                ...
                if ($erfolg>=1) {
                    ...
                    if ($minenanzahl>=1) {
                        ...
                        if ($aanomalie[2]<=0) {
                            $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where spiel=$spiel and id=$aanomalie[0]");
                            neuigkeiten(4,servername . "bilder/news/minenfeld.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][2],array($name));
                            neuigkeiten(4,servername . "bilder/news/minenfeld.jpg",$aanomalie[1],$lang['host'][$spielersprache[$aanomalie[1]]]['minenfelder'][3]);
                        } else {
                            neuigkeiten(4,servername . "bilder/news/minenfeld.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][4],array($name,$erfolg));
                            $mineextra=$aanomalie[1].':'.$aanomalie[2].':'.$aanomalie[3];
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set extra='$mineextra' where spiel=$spiel and id=$aanomalie[0]");
                        }
                    }
                ...
                neuigkeiten(4,servername . "bilder/news/minenfeld.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][7],array($name));
        ...
        if (($module[2]) and ($spezialmission==24)) {
            if($status!=2){
                ...
                if ($legen>=1) {
                    ...
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set projektile=$projektile,spezialmission=0,extra='$extra_neu' where id=$shid");
                    $zeiger_temp = mysql_query("INSERT INTO " . table_prefix . "anomalien (art,x_pos,y_pos,extra,spiel) values (5,$kox,$koy,'$mineextra',$spiel);");
                    neuigkeiten(4,servername . "bilder/news/minenfeld.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][5],array($name,$legen));
                } else {
                    $extra[2]=0;
                    $extra_neu = implode(":", $extra);
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0,extra='$extra_neu' where id=$shid");
                }
            }else{
                neuigkeiten(4, servername . "bilder/news/minenfeld.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['minenfelder'][6],array($name));
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0 where id=$shid");
            }
        }
                ...
                and ( ($spezialmission==4) or ($spezialmission==27) ) )
        {
            ...
            if ($max>=1) {
                ...
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_vorrat=fracht_vorrat-$vorrat_verbrauch,fracht_min1=fracht_min1+$min1_prod,fracht_min2=fracht_min2+$min2_prod,fracht_min3=fracht_min3+$min3_prod where id=$shid");
            }
        }
                ...
                if ($planetenanzahl==1) {
                    ...
                    if(($osys_1!=19) and ($osys_2!=19) and ($osys_3!=19) and ($osys_4!=19) and ($osys_5!=19) and ($osys_6!=19)){
                        if ($beziehung[$besitzer][$p_besitzer]['status']!=5) {
                            $sektork=sektor($kox,$koy);
                            $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "planeten where id=$p_id and besitzer=$p_besitzer;");
                            $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 'p:$p_id:%'");
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where flug=2 and zielid=$p_id");
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set status=1 where kox=$kox and koy=$koy and spiel=$spiel");
                            $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "sternenbasen where x_pos=$kox and y_pos=$koy and spiel=$spiel");
                            $suche=array('{1}','{2}');
                            $ersetzen=array($p_name,$sektork);
                            $text=str_replace($suche,$ersetzen,$text);
                            if (($spieler_1>=1) and ($p_besitzer<>1)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",1,$lang['host'][$spielersprache[1]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_2>=1) and ($p_besitzer<>2)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",2,$lang['host'][$spielersprache[2]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_3>=1) and ($p_besitzer<>3)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",3,$lang['host'][$spielersprache[3]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_4>=1) and ($p_besitzer<>4)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",4,$lang['host'][$spielersprache[4]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_5>=1) and ($p_besitzer<>5)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",5,$lang['host'][$spielersprache[5]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_6>=1) and ($p_besitzer<>6)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",6,$lang['host'][$spielersprache[6]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_7>=1) and ($p_besitzer<>7)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",7,$lang['host'][$spielersprache[7]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_8>=1) and ($p_besitzer<>8)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",8,$lang['host'][$spielersprache[8]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_9>=1) and ($p_besitzer<>9)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",9,$lang['host'][$spielersprache[9]]['destabilisator'][0],array($p_name,$sektork)); }
                            if (($spieler_10>=1) and ($p_besitzer<>10)) { neuigkeiten(4,servername . "bilder/news/star_explode.jpg",10,$lang['host'][$spielersprache[10]]['destabilisator'][0],array($p_name,$sektork)); }
                            if ($p_besitzer>=1) {
                                neuigkeiten(4,servername . "bilder/news/star_explode.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['destabilisator'][1],array($p_name,$sektork));
                            }
                        }
                    }else{
                        neuigkeiten(4,servername . "bilder/news/star_explode.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['destabilisator'][2],array($p_name,$sektork));
                    }
                }
            }
        }
        ...
        if ($fracht_vorrat>=220 && ($spezialmission==19 || $spezialmission==28) && $s_eigenschaften[$besitzer]['rasse']==$volk) {
            ...
            mysql_query("UPDATE " . table_prefix . "schiffe SET fracht_vorrat=fracht_vorrat-220, fracht_leute=fracht_leute+$kolonistengebaut WHERE id=$shid");
        }
        /////////////////////////////////////////////////////////////////////////////////////////////CYBERRITTNIKK ENDE
        ...
        if ( ($spezialmission==6) || ($spezialmission==26) ){
            ...
            if ($max>=1) {
                ...
                $lemin_prod=$max;
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_vorrat=fracht_vorrat-$vorrat_verbrauch,fracht_min1=fracht_min1-$min1_verbrauch,fracht_min2=fracht_min2-$min2_verbrauch,fracht_min3=fracht_min3-$min3_verbrauch,lemin=lemin+$lemin_prod where id=$shid");
            }
        }
        ...
        if (($spezialmission==2) and ($status==2)) {
            ...
            if ($planetenanzahl==1) {
                ...
                if ($p_sternenbasis_id>=1 or $osys_1==17 or $osys_2==17 or $osys_3==17 or $osys_4==17 or $osys_5==17 or $osys_6==17) {
                    ...
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=kolonisten+$fracht_leute,lemin=lemin+$fracht_lemin,min1=min1+$fracht_min1,min2=min2+$fracht_min2,min3=min3+$fracht_min3,vorrat=vorrat+$fracht_vorrat,cantox=cantox+$fracht_cantox where id=$p_id");
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set min1=min1+$neu_min1,min2=min2+$neu_min2,min3=min3+$neu_min3 where id=$p_id");
                    $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "schiffe where id=$shid");
                    $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where art=3 and extra like 's:$shid:%'");
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set flug=0,warp=0,zielx=0,ziely=0,zielid=0 where (flug=3 or flug=4) and zielid=$shid");
                    neuigkeiten(3,servername . "daten/$volk/bilder_basen/1.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['recycle'][0],array($name,$neu_min1,$neu_min2,$neu_min3));
                }
            }
        }
        ...
        if (($spezialmission==14) and ($status==2) and ($schaden>=1)) {
            ...
            if ($planetenanzahl==1) {
                ...
                if ($reperatur>=1) {
                    ...
                    if ($schaden>=1) {
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set schaden=$schaden where id=$shid");
                    } else {
                        $schaden=0;
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set schaden=0 where id=$shid");
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['reparatur'][0],array($name));
                    }
                }
            }
        }
        ...
        if (($spezialmission==23) and ($status==2)) {
            ...
            if ($planetenanzahl==1) {
                ...
                if ($p_sternenbasis_id>=1) {
                    ...
                    $extra[1]='';
                    $extra_neu = implode(":", $extra);
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set crew=$crew,extra='$extra_neu',spezialmission=0 where id=$shid");
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=$p_kolonisten where id=$p_id");
                    if($crew==$crewmax){
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['crew'][0],array($name,$leute_neu));
                    }else{
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['crew'][0],array($name,$leute_neu,$crew));
                    }
                }
            }
        }
        ...
        if (($spezialmission==1) and ($status==2)) {
            ...
            if ($planetenanzahl==1) {
                ...
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set lemin=lemin-$lemin_tanken where id=$p_id");
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set lemin=lemin+$lemin_tanken where id=$shid");
            }
        }
        /////////////////////////////////////////////////////////////////////////////////////////////TANKEN ENDE
        ...
        if (($spezialmission==3) and ($status==2)) {
            ...
            if ($planetenanzahl==1) {
                ...
                if ($beziehung[$besitzer][$p_besitzer]['status']!=5) {
                    ...
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=$p_kolonisten,minen=minen-$vernichtet_minen,fabriken=fabriken-$vernichtet_fabriken,abwehr=abwehr-$vernichtet_abwehr where id=$p_id");
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['bombardement'][0],array($name,$p_name,$vernichtet_minen,$prozent_minen,$vernichtet_fabriken,$prozent_fabriken,$vernichtet_abwehr,$prozent_abwehr,$vernichtet_kolonisten,$prozent_kolonisten));
                    neuigkeiten(1,servername . "bilder/planeten/$p_klasse"."_"."$p_bild.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['bombardement'][1],array($p_name,$vernichtet_minen,$prozent_minen,$vernichtet_fabriken,$prozent_fabriken,$vernichtet_abwehr,$prozent_abwehr,$vernichtet_kolonisten,$prozent_kolonisten));
                }
            }
        }
            ...
            if ($planetenanzahl==1) {
                ...
                if(($osys_1!=20) and ($osys_2!=20) and ($osys_3!=20) and ($osys_4!=20) and ($osys_5!=20) and ($osys_6!=20)){
                    if ($beziehung[$besitzer][$p_besitzer]['status']!=5) {
                        ...
                        $vernichtet_kolonisten=round($p_kolonisten/100*$prozent_kolonisten);
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=kolonisten-$vernichtet_kolonisten where id=$p_id");
                        neuigkeiten(2,servername . "bilder/news/epedemie.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['viral'][0],array($name,$p_name,$vernichtet_kolonisten,$prozent_kolonisten));
                        neuigkeiten(1,servername . "bilder/news/epedemie.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['viral'][1],array($p_name,$vernichtet_kolonisten,$prozent_kolonisten));
                    }
                }else{
                    neuigkeiten(2,servername . "bilder/news/epedemie.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['viral'][4],array($name,$p_name));
                    neuigkeiten(1,servername . "bilder/news/epedemie.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['viral'][5],array($p_name));
                }
            }
        }
            ...
            if ($planetenanzahl==1) {
                ...
                if(($osys_1!=20) and ($osys_2!=20) and ($osys_3!=20) and ($osys_4!=20) and ($osys_5!=20) and ($osys_6!=20)){
                    if ($beziehung[$besitzer][$p_besitzer]['status']!=5) {
                        ...
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set native_kol=native_kol-$vernichtet_native where id=$p_id");
                        neuigkeiten(2,servername . "bilder/news/epedemie.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['viral'][2],array($name,$p_name,$vernichtet_native,$prozent_native));
                        if ($p_besitzer>=1) {
                            neuigkeiten(1,servername . "bilder/news/epedemie.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['viral'][3],array($p_name,$vernichtet_native,$prozent_native));
                        }
                    }
                }else{
                    neuigkeiten(2,servername . "bilder/news/epedemie.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['viral'][4],array($name,$p_name));
                    if ($p_besitzer>=1) {
                        neuigkeiten(1,servername . "bilder/news/epedemie.jpg",$p_besitzer,$lang['host'][$spielersprache[$p_besitzer]]['viral'][6],array($p_name));
                    }
                }
            }
        }
        ...
        if (($spezialmission==5) and ($status==2)) {
            ...
            if ($planetenanzahl==1) {
                ...
                if ($fert_terra_warm>=1) {
                    ...
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['terraforming'][0],array($p_name,$fert_terra_warm,$tempschreib));
                }
                if ($fert_terra_kalt>=1) {
                    $p_temp=$p_temp-$fert_terra_kalt;
                    $tempschreib=$p_temp-35;
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['terraforming'][1],array($p_name,$fert_terra_kalt,$tempschreib));
                }
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set temp=$p_temp where id=$p_id");
            }
        }
        /////////////////////////////////////////////////////////////////////////////////////////////TERRAFORMING ENDE
        /////////////////////////////////////////////////////////////////////////////////////////////SPRUNGTOR ANFANG
        if (($spezialmission==13) and ($status==1) and ($flug==0)) {
            ...
            if ($p2anzahl>=1) {
                $ok=1;
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtor'][0]);
            }else{
                ...
                if ($a2anzahl>=1) {
                    $ok=1;
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtor'][1]);
                }else{
                    if (($fert_sprungtorbau_min1>$fracht_min1) or ($fert_sprungtorbau_min2>$fracht_min2) or ($fert_sprungtorbau_min3>$fracht_min3) or ($fert_sprungtorbau_lemin>$fracht_lemin)) {
                        $ok=1;
                        neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtor'][2]);
                    }
                }
            }
            if ($ok==2) {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_min1=fracht_min1-$fert_sprungtorbau_min1,fracht_min2=fracht_min2-$fert_sprungtorbau_min2,fracht_min3=fracht_min3-$fert_sprungtorbau_min3,lemin=lemin-$fert_sprungtorbau_lemin where id=$shid");
                if ($sprungtorbauid>=1) {
                    ...
                    $zeiger2 = mysql_query("INSERT INTO " . table_prefix . "anomalien (art,x_pos,y_pos,extra,spiel) values (2,$kox,$koy,'$extra',$spiel);");
                    ...
                    $zeiger2 = mysql_query("UPDATE " . table_prefix . "anomalien set extra='$extra' where id=$sprungtorbauid;");
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set sprungtorbauid=0,spezialmission=0 where id=$shid");
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtor'][3]);
                } else {
                    $zeiger_temp = mysql_query("INSERT INTO " . table_prefix . "anomalien (art,x_pos,y_pos,spiel) values (2,$kox,$koy,$spiel);");
                    $zeiger_temp = mysql_query("SELECT * FROM " . table_prefix . "anomalien where x_pos=$kox and y_pos=$koy and spiel=$spiel");
                    $array_temp = mysql_fetch_array($zeiger_temp);
                    $aid=$array_temp["id"];
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set sprungtorbauid=$aid,spezialmission=0 where id=$shid");
                    neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['sprungtor'][4]);
                }
            }
        }
        /////////////////////////////////////////////////////////////////////////////////////////////SPRUNGTOR ENDE
                ...
                if($planeten_anzahl==1){
                    ...
                    if($fert_akademie==1){
                        if($spezialmission==72 and $fracht_cantox>=100 and $fracht_vorrat>=10 and $fracht_lemin>=50){
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_cantox=fracht_cantox-100,fracht_vorrat=fracht_vorrat-10,lemin=lemin-50,erfahrung=erfahrung+1,spezialmission=0 where id=$shid and erfahrung<5 and spiel=$spiel");
                            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['akademie'][2],array($name));
                        }elseif($spezialmission>72 and $spezialmission<77){
                            $spezialmission--;
                            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=$spezialmission where id=$shid and spiel=$spiel");
                            if($spezialmission>72){
                                neuigkeiten(2, servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['akademie'][0],array($name,$spezialmission-71));
                            }else{
                                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['akademie'][1],array($name));
                            }
                        }
                    }
                }
            }else{
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set spezialmission=0 where id=$shid and spiel=$spiel");
                neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['akademie'][3],array($name));
            }
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////TARNFELD ANFANG
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=1 where spezialmission=8 and spiel=$spiel");
if($module[0]) {
    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where spezialmission<>8 and !(volk='unknown' and klasseid=1) and spiel=$spiel");
    ...
    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where spezialmission<>8 and spiel=$spiel");
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($min2_brauch<=$fracht_min2) {
            ...
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_min2=$fracht_min2 where id=$shid");
        } else {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where id=$shid");
        }
    }
}
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($zuzahl<=19) {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where id=$shid");
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['tarnfeld'][0],array($name));
        }
    }
}
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($zuzahl<=($antrieb_anzahl*2)) {
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['drugun'][2],array($name));
            $reichweite=117;
            if($module[0]) {
                ...
                if ($treffschiff>=1) {
                    for ($k=0; $k<$treffschiff;$k++) {
                        ...
                        $t_volk=$array_temp["volk"];
                        $t_bild_gross=$array_temp["bild_gross"];
                        $zeiger_temp2 = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where id=$t_shid and spiel=$spiel");
                        neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugun'][0],array($t_name));
                    }
                }
                ...
                if($schiffanzahl>=1) {
                    for($i=0; $i<$schiffanzahl;$i++) {
                        ...
                        if($tarnfeld<=0) {
                            $tarnfeld = 0;
                            neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugun'][1],array($t_name));
                        }else{
                            neuigkeiten(2, servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugun'][0],array($t_name));
                        }
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=$tarnfeld where id=$t_shid");
                    }
                }
            }else {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=0 where (sqrt(((kox-$kox)*(kox-$kox))+((koy-$koy)*(koy-$koy)))<=$reichweite) and spiel=$spiel and tarnfeld=1");
            }
        }
    }
}
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=1 where antrieb=2 and spiel=$spiel");
/////////////////////////////////////////////////////////////////////////////////////////////TARNFELD ENDE
/////////////////////////////////////////////////////////////////////////////////////////////DRUGUNVERZERRER ANFANG
...
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($zufall<=67) {
            ...
            neuigkeiten(2,servername . "daten/$volk/bilder_schiffe/$bild_gross",$besitzer,$lang['host'][$spielersprache[$besitzer]]['drugun'][5],array($name,$reichweite));
            $zeiger2 = mysql_query("SELECT * FROM " . table_prefix . "planeten where x_pos=$kox and y_pos=$koy and besitzer=$besitzer and spiel=$spiel");
            $planetenanzahl = mysql_num_rows($zeiger2);
            if ($planetenanzahl==1) {
                $array2 = mysql_fetch_array($zeiger2);
                ...
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=$p_kolonisten,leichtebt=$p_leichtebt,schwererbt=$p_schwerebt where x_pos=$kox and y_pos=$koy and besitzer=$besitzer and spiel=$spiel");
                ...
                if ($treffschiff>=1) {
                    for($i=0; $i<$treffschiff;$i++) {
                        ...
                        $tarnfeld--;
                        if($tarnfeld==0){
                            neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugun'][3],array($t_name));
                        }else{
                            neuigkeiten(2,servername . "daten/$t_volk/bilder_schiffe/$t_bild_gross",$t_besitzer,$lang['host'][$spielersprache[$t_besitzer]]['drugun'][4],array($t_name));
                        }
                        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=$tarnfeld where id=$t_shid");
                    }
                }
...
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set scanner=0 where spezialmission<>11 and spezialmission<>12 and spiel=$spiel");
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set scanner=1 where spezialmission=11 and spiel=$spiel");
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set scanner=2 where spezialmission=12 and spiel=$spiel");
...
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        ...
        if ($kolonisten<1) {
            ...
            if ($weg==1) {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set leichtebt=0,schwerebt=0,kolonisten=0,besitzer=0,auto_minen=0,auto_fabriken=0,abwehr=0,auto_abwehr=0,auto_vorrat=0,vorrat=0,logbuch='' where id=$pid");
                if ($sternenbasis_id>=1) {
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "sternenbasen set besitzer=0 where id=$sternenbasis_id");
                }
            } else {
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set vorrat=$vorrat,leichtebt=$leichtebt,schwerebt=$schwerebt where id=$pid");
            }
        } else {
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set kolonisten=$kolonisten where id=$pid");
        }
    }
}
...
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        ...
        $sternenbasis_id=$array["sternenbasis_id"];
        $zeiger_temp = mysql_query("update " . table_prefix . "planeten set leichtebt=$leichtebt_new,schwerebt=$schwerebt_new,leichtebt_new=0,schwerebt_new=0,besitzer=$kolonisten_spieler,kolonisten=$kolonisten_new,kolonisten_new=0,kolonisten_spieler=0 where id=$pid");
        if ($sternenbasis_id>=1) { $zeiger_temp = mysql_query("UPDATE " . table_prefix . "sternenbasen set besitzer=$kolonisten_spieler where id=$sternenbasis_id;"); }
        if ($kolonisten_new>0) {
            $neuekolonie++;
            neuigkeiten(1, servername . "bilder/planeten/$klasse"."_"."$bild.jpg",$kolonisten_spieler,$lang['host'][$spielersprache[$kolonisten_spieler]]['besetzen'][0],array($name));
        } else {
            neuigkeiten(1, servername . "bilder/planeten/$klasse"."_"."$bild.jpg",$kolonisten_spieler,$lang['host'][$spielersprache[$kolonisten_spieler]]['besetzen'][1],array($name));
        }
    }
}
...
if ($meteor==1) {
    ...
    if ($besitzer>=1) {
        ...
        for ($k=1;$k<11;$k++) {
            if (($spieler_id_c[$k]>=1) and ($spieler_raus_c[$k]!=1) and ($besitzer!=$k)) { neuigkeiten(4,servername . "bilder/news/meteor_gross.jpg",$k,$lang['host'][$spielersprache[$k]]['meteoriten'][0],array($name,$x_pos,$y_pos,$rohstoffe[0],$rohstoffe[2],$rohstoffe[1],$rohstoffe[3])); }
        }
        neuigkeiten(4, servername . "bilder/news/meteor_gross.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['meteoriten'][1],array($name,$x_pos,$y_pos,$rohstoffe[0],$rohstoffe[2],$rohstoffe[1],$rohstoffe[3]));
    } else {
        $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set planet_lemin=planet_lemin+$lemin,planet_min1=planet_min1+$min1,planet_min2=planet_min2+$min2,planet_min3=planet_min3+$min3,native_id=0,native_name ='',native_art=0,native_art_name='',native_abgabe=0,native_bild='',native_text='',native_fert='',native_kol=0 where id=$pid");
        for ($k=1;$k<11;$k++) {
            if (($spieler_id_c[$k]>=1) and ($spieler_raus_c[$k]!=1)) { neuigkeiten(4,servername . "bilder/news/meteor_gross.jpg",$k,$lang['host'][$spielersprache[$k]]['meteoriten'][0],array($name,$x_pos,$y_pos,$rohstoffe[0],$rohstoffe[2],$rohstoffe[1],$rohstoffe[3])); }
        }
    }
}
...
for ($i=0; $i<$meteore;$i++) {
    ...
    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set lemin=$lemin,min1=$min1,min2=$min2,min3=$min3 where id=$pid");
    if ($besitzer>=1) {
        neuigkeiten(2, servername . "bilder/news/meteor_klein.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['meteoriten'][2],array($name,$rohstoffe[0],$rohstoffe[2],$rohstoffe[1],$rohstoffe[3]));
    }
}
                ...
                if ($zufall<=$wahrscheinlichkeit) {
                    ...
                    if ($prozent_ganz>=1) {
                        ...
                        if (($fracht_min1_weg>=1) or ($fracht_min2_weg>=1) or ($fracht_min3_weg>=1) or ($fracht_cantox_weg>=1) or ($fracht_vorrat_weg>=1)) {
                            mysql_query("UPDATE " . table_prefix . "schiffe set fracht_min1=fracht_min1-$fracht_min1_weg,fracht_min2=fracht_min2-$fracht_min2_weg,fracht_min3=fracht_min3-$fracht_min3_weg,fracht_cantox=fracht_cantox-$fracht_cantox_weg,fracht_vorrat=fracht_vorrat-$fracht_vorrat_weg where id=$shid");
                            neuigkeiten(1,servername . "bilder/news/piraten.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['piraten'][0],array($name,$fracht_cantox_weg,$fracht_vorrat_weg,$fracht_min1_weg,$fracht_min2_weg,$fracht_min3_weg));
                        }
                    } else {
                        neuigkeiten(1, servername . "bilder/news/piraten.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['piraten'][1],array($name));
                    }
                } else {
...
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        ...
        if (($native_id>=1) and ($native_kol>1) and ($native_fert_prod_vorrat>0)) {
            ...
            $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set vorrat=$vorrat where id=$pid");
        }
        if (($native_id>=1) and ($native_kol>1) and ($native_fert_prod_lemin>0)) {
            $lemin=$lemin+round($native_kol/10000*$native_fert_prod_lemin);
            $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set lemin=$lemin where id=$pid");
        }
        $zufall=mt_rand(1,100);
        if ($zufall<=18) {
            if ($artefakt==1) {
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set artefakt=0 where id=$pid");
                $zufall2=mt_rand(500,1500);
                $planet_lemin=$planet_lemin+$zufall2;
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set planet_lemin=$planet_lemin where id=$pid");
                neuigkeiten(1,servername. "bilder/news/vorkommen_lemin.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['mineralader'],array($name,'Lemin'));
            }
            if ($artefakt==2) {
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set artefakt=0 where id=$pid");
                $zufall2=mt_rand(500,1500);
                $planet_min1=$planet_min1+$zufall2;
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set planet_min1=$planet_min1 where id=$pid");
                neuigkeiten(1,servername. "bilder/news/vorkommen_min1.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['mineralader'],array($name,'Baxterium'));
            }
            if ($artefakt==3) {
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set artefakt=0 where id=$pid");
                $zufall2=mt_rand(500,1500);
                $planet_min2=$planet_min2+$zufall2;
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set planet_min2=$planet_min2 where id=$pid");
                neuigkeiten(1,servername. "bilder/news/vorkommen_min2.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['mineralader'],array($name,'Rennurbin'));
            }
            if ($artefakt==4) {
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set artefakt=0 where id=$pid");
                $zufall2=mt_rand(500,1500);
                $planet_min3=$planet_min3+$zufall2;
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set planet_min3=$planet_min3 where id=$pid");
                neuigkeiten(1,servername. "bilder/news/vorkommen_min3.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['mineralader'],array($name,'Vomisaan'));
            }
        }
        if ($zufall<=33) {
            if (($artefakt==6) and ($osys_anzahl<6)) {
                ...
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set $spalte=14,osys_anzahl=$osys_anzahl where id=$pid");
                neuigkeiten(1,servername. "bilder/news/wetterstation.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['wetterstation'],array($name));
            }
        }
        if ($zufall<=50) {
            if ($artefakt==5) {
                ...
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=$kolonisten where id=$pid");
                neuigkeiten(1,servername. "bilder/news/splitterkolonie.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['kolonisten'],array($name,$zufall2));
            }
        }
        ...
        if (in_array (14, $osys)) {
            if ($temp!=$r_eigenschaften[$rasse]['temperatur']) {
                ...
                $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set temp=$temp where id=$pid");
            }
        }
        ...
        if (($leichtebt_bau>=1) or ($schwerebt_bau>=1)) {
            ...
            $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set leichtebt=$leichtebt,leichtebt_bau=0,schwerebt=$schwerebt,schwerebt_bau=0 where id=$pid");
        }
        ...
        if (($osys_1==5) or ($osys_2==5) or ($osys_3==5) or ($osys_4==5) or ($osys_5==5) or ($osys_6==5)) {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "schiffe set tarnfeld=1 where status=2 and kox=$x_pos and koy=$y_pos and spiel=$spiel");
        }
        ...
        if (($native_id>=1) and ($native_kol>1) and ($native_fert_attacke>0)) {
            ...
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set native_kol=$native_kol,kolonisten=$kolonisten,leichtebt=$leichtebt,schwerebt=$schwerebt where id=$pid");
        }
        //NATIVE ENDE KAMPF
        $Bankbonus=1;
        ...
        if (($native_id>=1) and ($native_kol>1) and ($native_fert_angriffswarnung==1)) {
            ...
            if ($anzahl_angreifer>0) {
                neuigkeiten(1,servername. "bilder/planeten/$klasse"."_"."$bild.jpg",$besitzer,$lang['host'][$spielersprache[$besitzer]]['angriffswarnung'],array($name));
            }
        }
        ...
        if (($native_id>=1) and ($native_kol<1000000)) {
            ...
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set native_kol=$native_kol where id=$pid");
        }
        ...
        if (($native_id>=1) and ($native_kol>1) and ($r_eigenschaften[$rasse]['assgrad']>=1) and ($kolonisten>1)) {
            if (($r_eigenschaften[$rasse]['assart']==$native_art) or ($r_eigenschaften[$rasse]['assart']==0)) {
                ...
                if ($ueberlauf>=1 and $native_kol>$reservat) {
                    ...
                    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set kolonisten=$kolonisten,native_id=$native_id,native_kol=$native_kol where id=$pid");
                }
            }
        }
        ...
        $zeiger_temp = mysql_query("update " . table_prefix . "planeten set cantox=$cantox where id=$pid");
        //KOLONISTEN ENDE
        ...
        $zeiger_temp = mysql_query("update " . table_prefix . "planeten set vorrat=$vorrat where id=$pid");
        ...
        if ($minen>0) {
            ...
            if ($mineralgesamt>=1) {
                ...
                //Vormissan Ende
                $zeiger_temp = mysql_query("UPDATE " . table_prefix . "planeten set lemin=$lemin,min1=$min1,min2=$min2,min3=$min3,planet_lemin=$planet_lemin,planet_min1=$planet_min1,planet_min2=$planet_min2,planet_min3=$planet_min3 where id=$pid");
            }
        }
        ...
        if ($auto_fabriken==1) {
            ...
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set fabriken=$fabriken,cantox=$cantox,vorrat=$vorrat where id=$pid");
        }
        ...
        if ($prozent>$max_col) { $fabriken=$prozent; } else { $fabriken=$max_col; }
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set fabriken=$fabriken where id=$pid");
        }
        //FABRIKENABBAU ENDE
        ...
        if ($auto_minen==1) {
            ...
            $vorrat=$vorrat-$max_bau;
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set minen=$minen,cantox=$cantox,vorrat=$vorrat where id=$pid");
        }
        ...
        if ($minen>$max_col) {
            ...
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set minen=$minen where id=$pid");
        }
        //MINENABBAU ENDE
        ...
        if ($auto_abwehr==1) {
            ...
            $vorrat=$vorrat-$max_bau;
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set abwehr=$abwehr,cantox=$cantox,vorrat=$vorrat where id=$pid");
        }
        ...
        if ($abwehr>$max_col) {
            ...
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set abwehr=$abwehr where id=$pid");
        }
        ...
        if ($auto_vorrat==1) {
            ...
            $vorrat=0;
            $zeiger_temp = mysql_query("update " . table_prefix . "planeten set vorrat=$vorrat,cantox=$cantox where id=$pid");
        }
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($planetenanzahl==1) {
            ...
            $s_lbt=$r_planet[6];
            $s_sbt=$r_planet[7];
            $zeigertemp = mysql_query("UPDATE " . table_prefix . "planeten set lemin=$p_lemin,cantox=$s_cantox,vorrat=$s_vorrat,min1=$s_bax,min2=$s_ren,min3=$s_vor,kolonisten=$s_kol,leichtebt=$s_lbt,schwerebt=$s_sbt where id=$p_id");
            $s_cantox=$r_fracht[0];
            $s_vorrat=$r_fracht[1];
            $s_bax=$r_fracht[2];
            $s_ren=$r_fracht[3];
            ...
            $s_lbt=$r_fracht[6];
            $s_sbt=$r_fracht[7];
            $zeigertemp = mysql_query("UPDATE " . table_prefix . "schiffe set fracht_leute=$s_kol,leichtebt=$s_lbt,schwerebt=$s_sbt,lemin=$fracht_lemin,fracht_vorrat=$s_vorrat,fracht_cantox=$s_cantox,fracht_min1=$s_bax,fracht_min2=$s_ren,fracht_min3=$s_vor where id=$shid");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////ROUTEBEAMEN ENDE
...
///////////////////////////////////////////////////////////////////////////////////////////////TRAKTORSTRAHL UEBERPRUEFEN ENDE
///////////////////////////////////////////////////////////////////////////////////////////////POLITIKENDE ANFANG
$zeiger = mysql_query("DELETE FROM " . table_prefix . "politik where optionen=1 and spiel=$spiel");
$zeiger = mysql_query("UPDATE " . table_prefix . "politik set optionen=optionen-1 where optionen>1 and spiel=$spiel");
///////////////////////////////////////////////////////////////////////////////////////////////POLITIKENDE ENDE
...
if ($datensaetze>=1) {
    for ($i=0; $i<$datensaetze;$i++) {
        ...
        $zeit--;
        if ($zeit>=1) {
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set extra='$zeit' where id=$aid");
        } else {
            if($zeit==0){
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "anomalien where id=$aid");
            }else{}
        }
    }
}
...
if (($sturm<$plasma_max) and ($zufall<=$plasma_wahr)) {
            ...
            if($zufall<=(100-($abstand*5))){
                ...
                if($reihen>=1){
                    ...
                    if($zeit==-1){
                        ...
                        $zeiger_temp = mysql_query("INSERT INTO " . table_prefix . "anomalien (art,x_pos,y_pos,extra,spiel) values (4,($x+$i)*10,($y+$j)*10,'$runden',$spiel)");
                    }
                }else{
                    $runden=mt_rand(3,$plasma_lang);
                    $plasma_lang_max=max($runden,$plasma_lang_max);
    ...
    $zeiger_temp = mysql_query("INSERT INTO " . table_prefix . "anomalien (art,extra,spiel) values (6,'$plasma_lang_max',$spiel)");
}
...
include(inhalt_dir . 'inc.host_nebel.php');
///////////////////////////////////////////////////////////////////////////////////////////////NEBELSEKTOREN ENDE
...
if ($schiffanzahl>=1) {
    for ($i=0; $i<$schiffanzahl;$i++) {
        ...
        if ($zielanzahl==1) {
            $zeigertemp = mysql_query("update " . table_prefix . "schiffe set flug=0,zielx=0,ziely=0,zielid=0 where id=$ssid");
        }
    }
}
...
if ($module[2]) {
    $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set sicht='0000000000' where (art=5) and spiel=$spiel");
    ...
    if ($datensaetze>=1) {
        for ($i=0; $i<$datensaetze;$i++) {
            ...
            /////////////////////
             $reichweite=161;
            $zeiger_temp = mysql_query("SELECT kox,koy,id,scanner,spiel,besitzer FROM " . table_prefix . "schiffe where besitzer!=$extrab[0] and (sqrt(((kox-$kox)*(kox-$kox))+((koy-$koy)*(koy-$koy)))<=$reichweite) and spiel=$spiel order by id");
            ...
            $zeiger_temp = mysql_query("UPDATE " . table_prefix . "anomalien set sicht='$sicht' where id=$aid and spiel=$spiel");
        }
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////MINEN SICHTBAR ENDE
...
$zeiger = mysql_query("SELECT id,besitzer,spiel,heimatplanet FROM " . table_prefix . "planeten where spiel=$spiel and besitzer>=1 order by id");
$planetenanzahl = mysql_num_rows($zeiger);
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
...
$zeiger = mysql_query("SELECT id,besitzer,spiel FROM " . table_prefix . "sternenbasen where spiel=$spiel and besitzer>=1 order by id");
$planetenanzahl = mysql_num_rows($zeiger);
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
...
$zeiger = mysql_query("SELECT id,besitzer,techlevel,spiel FROM " . table_prefix . "schiffe where spiel=$spiel and besitzer>=1 order by id");
$planetenanzahl = mysql_num_rows($zeiger);
if ($planetenanzahl>=1) {
    for ($i=0; $i<$planetenanzahl;$i++) {
        $ok = mysql_data_seek($zeiger,$i);
...
for ($m=1;$m<11;$m++) {
    if (($spieler_raus_c[$m]==1) and ($spieler_raus_c_old[$m]==0)) {
        $zeiger = mysql_query("SELECT besitzer,id,spiel FROM " . table_prefix . "sternenbasen where besitzer=$m and spiel=$spiel order by id");
        $basenanzahl = mysql_num_rows($zeiger);
        if ($basenanzahl>=1) {
            for ($i=0; $i<$basenanzahl;$i++) {
                $ok = mysql_data_seek($zeiger,$i);
                $array = mysql_fetch_array($zeiger);
                $baid=$array["id"];
                $zeiger_temp = mysql_query("DELETE FROM " . table_prefix . "huellen where baid=$baid;");
            }
        }
        $zeiger = mysql_query("UPDATE " . table_prefix . "sternenbasen set besitzer=0 where besitzer=$m and spiel=$spiel");
        $zeiger = mysql_query("SELECT * FROM " . table_prefix . "schiffe where besitzer=$m and spiel=$spiel");
        $schiffanzahl = mysql_num_rows($zeiger);
        if ($schiffanzahl>=1) {
            for ($i=0; $i<$schiffanzahl;$i++) {
                $ok = mysql_data_seek($zeiger,$i);
...
$zeiger_temp = mysql_query("UPDATE " . table_prefix . "spiele set letztermonat='$letztermonat', runde=runde+1 where id=$spiel;");
///////////////////////////////////////////////////////////////////////////////////////////////ZIEL ANFANG
$endejetzt=0;
if ($ziel_id==1) {
...
if ($ziel_id==5) {
    ...
    $zeiger = mysql_query("SELECT status,spiel,id,besitzer,fracht_min3 FROM " . table_prefix . "schiffe where status<>2 and spiel=$spiel order by id");
    $schiffanzahl = mysql_num_rows($zeiger);
    if ($schiffanzahl>=1) {
        for ($i=0; $i<$schiffanzahl;$i++) {
            $ok = mysql_data_seek($zeiger,$i);
    ...
    $zeiger_temp = mysql_query("UPDATE " . table_prefix ."spiele set spieler_1_ziel='$spieler_ziel_t_c[1]',spieler_2_ziel='$spieler_ziel_t_c[2]',spieler_3_ziel='$spieler_ziel_t_c[3]',spieler_4_ziel='$spieler_ziel_t_c[4]',spieler_5_ziel='$spieler_ziel_t_c[5]',spieler_6_ziel='$spieler_ziel_t_c[6]',spieler_7_ziel='$spieler_ziel_t_c[7]',spieler_8_ziel='$spieler_ziel_t_c[8]',spieler_9_ziel='$spieler_ziel_t_c[9]',spieler_10_ziel='$spieler_ziel_t_c[10]' where id=$spiel;");
...
if ($endejetzt==1) {
    include(inhalt_dir.'inc.host_spielende.php');
}
///////////////////////////////////////////////////////////////////////////////////////////////ZIEL ENDE
///////////////////////////////////////////////////////////////////////////////////////////////STATS AUSWERTUNG ANFANG
for ($m=1;$m<11;$m++) {
    if ($spieler_id_c[$m]>=1) {
                               $zeiger = mysql_query("UPDATE " . table_prefix ."user set stat_sieg=stat_sieg+" . $stat_sieg[$m] .",
...
$aktuell=time();
$zeiger = mysql_query("INSERT INTO " . table_prefix ."chat (spiel,datum,text,an,von,farbe) values ('" . $spiel . "','" . $aktuell ."','" . $nachricht ."','0','System','000000')");
